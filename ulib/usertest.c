...
int safe_open(const char *file, const int lineno, void (*cleanup_fn)(void), const char *filename,
              int oflags) {
    int fd = open(filename, oflags, I_RWX);
    if (fd < 0) {
        printf("%s:%d: open(%s, %d) failed, return value %d\n", file, lineno, filename, oflags, fd);
        if (cleanup_fn) { (*cleanup_fn)(); }
        exit(-1);
    }
    return fd;
...
int safe_write(const char *file, const int lineno, int fd, const void *buf, int nbytes) {
    int n = write(fd, buf, nbytes);
    if (n < 0) {
        printf("%s:%d: write(%d, %p, %d) failed, return value %d\n", file, lineno, fd, buf, nbytes,
               n);
        // if (cleanup_fn != NULL)
        // cleanup_fn();
        exit(-1);
...
int safe_read(const char *file, const int lineno, int fd, void *buf, int size) {
    ...
    if (rval < 0) {
        printf("%s:%d: read(%d, %p, %d) failed, return %d", file, lineno, fd, buf, size, rval);
        // if (cleanup_fn != NULL)
        // cleanup_fn();
        exit(-1);
    }
...
int logger_init(logging *logger, const char *log_filename, const char *test_name,
                logger_level level) {
    int fd;
    ...
    logger->log_filename = (char *)log_filename;
    logger->logfd = fd;
    logger->test_name = (char *)test_name;
    logger->level = level;
...
int check_logger(logging *logger) {
    return logger && logger->logfd >= 0 && logger->level >= LOG_DEBUG && logger->level <= LOG_ERROR;
}

// real output function
int logger_write(logger_level current_level, logging *logger, const char *fmt, va_list args) {
    int FMT_SIZE = 4096;
    int nbytes = 0;
    ...
    sprintf(real_fmt, "%s: %s: %s", logger->test_name, LOG_LEVEL_LITERAL[current_level], fmt);
...
int logger_print(logger_level current_level, logging *logger, const char *fmt, va_list args) {
    ...
    if (!check_logger(logger)) {
        ...
        // printf(
        // "logging: logger: %d, fd: %d, level: %d, filename: %s, testname:
    ...
    if (current_level < logger->level) { return 0; }
