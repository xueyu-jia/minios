...
void produce1(void *arg) {
    int fd = *(int *)arg;
    for (int i = 0; i < 50; i++) { write(fd, bufw1, strlen(bufw1)); }
...
void produce2(void *arg) {
    int fd = *(int *)arg;
    for (int i = 0; i < 50; i++) { write(fd, bufw2, strlen(bufw2)); }
    pthread_mutex_lock(&mutex);
    ++sum;
    pthread_mutex_unlock(&mutex);
...
void produce3(void *arg) {
    int fd = *(int *)arg;
    for (int i = 0; i < 50; i++) { write(fd, bufw3, strlen(bufw3)); }
    pthread_mutex_lock(&mutex);
    ++sum;
    pthread_mutex_unlock(&mutex);
...
int main(int arg, char *argv[]) {
    ...
    if (fd == -1) { fd = open(filename, O_RDWR); }
    if (fd == -1) {
        printf("open1 error!");
        exit(-1);
    }
    ...
    // 删除文件
    if (unlink(filename)) { printf("delete file error!"); }
    exit(0);
