...
void run() {
    ...
    while (start_count < THREAD_NUM) { sleep(10); }
    sleep(10);
    ...
    if (rval != 0) {
        error(&logger, "main thread: pthread_cond_broadcast return %d, expected %d\n", rval, 0);
        cleanup();
        exit(TC_FAIL);
    }
    sleep(1000);
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        ...
        if (rval != 0) { warning(&logger, "main thread: pthread_join return %d\n", rval); }
    }
