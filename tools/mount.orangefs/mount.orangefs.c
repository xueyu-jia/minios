...
#define OPTION(t, p) {t, offsetof(struct orangefs_opts, p), 1}
static const struct fuse_opt option_spec[] = {OPTION("-h", show_help), OPTION("--help", show_help),
                                              FUSE_OPT_END};
...
void orangefs_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi) {
    struct stat stbuf;
...
void orangefs_rename(fuse_req_t req, fuse_ino_t parent, const char *name, fuse_ino_t newparent,
    ...
    if (old_ino == 0 || name[0] == '\0' || newname[0] == '\0') { fuse_reply_err(req, -ENOENT); }
    if (old_ino == new_ino) { fuse_reply_err(req, 0); }
    switch (flags) {
...
void orangefs_forget(fuse_req_t req, fuse_ino_t ino, uint64_t nlookup) {
    if (nlookup == 0) {
        ...
        // pin->i_start_block+pin->i_nr_blocks);
        // }
...
void orangefs_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
    ...
    if (ino <= 0)
        ...
        if (dir == NULL) { fuse_reply_err(req, -ENOENT); }
        if ((dir->i_mode & I_TYPE_MASK) != I_DIRECTORY) { fuse_reply_err(req, ENOTDIR); }
        struct dirbuf b;
...
void orangefs_write(fuse_req_t req, fuse_ino_t ino, const char *buf, size_t size, off_t off,
                    struct fuse_file_info *fi) {
    struct orange_inode *pin = orangefs_iget(ino);
    if (pin == 0) fuse_reply_err(req, -ENOENT);
    if ((pin->i_mode & I_TYPE_MASK) == I_DIRECTORY) fuse_reply_err(req, EISDIR);
...
void orangefs_create(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode,
                     struct fuse_file_info *fi) {
    int ino = orangefs_find(parent, name, NULL);
    if (ino != 0) { fuse_reply_err(req, -EEXIST); }
...
void orangefs_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode) {
    int ino = orangefs_find(parent, name, NULL);
    if (ino != 0) { fuse_reply_err(req, -EEXIST); }
    ino = orangefs_allocinode();
    struct orange_inode *pin = orangefs_iget(ino);
...
static int orangefs_opt_proc(void *data, const char *arg, int key, struct fuse_args *outargs) {
    (void)data;
    (void)outargs;
    ...
    switch (key) {
        ...
        case FUSE_OPT_KEY_NONOPT:
            if (!options.disk) {
                ...
                return 0; // discard input disk (first argv), second is
                          // mountpoint
            }
            return 1;
        default:
...
int main(int argc, char *argv[]) {
    ...
    options.disk = NULL;
    /* Parse options */
    if (fuse_opt_parse(&args, &options, option_spec, orangefs_opt_proc) == -1) return 1;
    ...
    int multithread = 0;
    if (fuse_parse_cmdline(&args, &opts.mountpoint, &multithread, &opts.foreground) != 0) return 1;
...
#else
    se = fuse_session_new(&args, &orangefs_oper, sizeof(orangefs_oper), NULL);
    if (se == NULL) goto err_out1;
    fuse_session_mount(se, opts.mountpoint);
#endif

    if (fuse_set_signal_handlers(se) != 0) goto err_out;
