...
int orangefs_fillsuper() {
    ...
    if (superblock.magic != ORANGE_MAGIC) { return -1; }
    return 0;
}
...
void orangefs_allocsector(struct orange_inode *pin) {
    ...
    pin->i_nr_blocks = orangefs_alloc_bitmap(2 + superblock.nr_imap_blocks,
...
void orangefs_iforget_may_drop(int ino, struct orange_inode *inode) {
    ...
    list_for_each(&inode_list, ic, ilist) {
        if (ic->iptr == inode) { target = ic; }
    }
    if (target) { orangefs_clear_inodecache(target); }
    pthread_mutex_unlock(&cache_lock);
}
...
int orangefs_add_direntry(int parent, const char *name, int ino) {
    ...
    for (blk = dir->i_start_block; blk < dir->i_start_block + dir->i_nr_blocks; blk++) {
        for (pos = 0; pos < ORANGE_BLK_SIZE; pos += ORANGE_DIRENT_SIZE) {
            disk_read(blk, pos, ORANGE_DIRENT_SIZE, &dirent);
            ...
            if (dirent.inode_nr == 0) { goto findfree; }
        }
        p += ORANGE_BLK_SIZE;
    }
    if (blk == dir->i_start_block + dir->i_nr_blocks) { return -1; }
findfree:
...
int orangefs_stat(int ino, struct stat *buf) {
    ...
    buf->st_mode = 0777 | ((m == I_DIRECTORY) ? S_IFDIR : ((m == I_REGULAR) ? S_IFREG : 0));
    return 0;
}
