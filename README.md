...
下文过度陈旧，但是没时间更新，这里之提下 v2.0.0.alpha.1 之后重要的几点。
...
- `build/<arch>/` 当前配置架构的生成文件的目录，原则是凡是架构相关的生成内容都在这
...
install 仅装载相关目标到系统镜像，镜像仅在必要时自动生成，如尚不存在或 bootloader 及 kernel 更新。
...
构建系统极其复杂且特立独行，如果不是出于学习目的或纯粹愿意下精力研究，建议别看。事实上，你几乎无法再碰到什么情况需要你乃至微调构建过程的需求，甚至你可能难以找到当前项目中的一些具体事务在构建系统的哪里得到体现——它足够泛用了。

其次，如果你只是想调调某些参数，更别动 \*.mk！那里提供了相对可靠的一套默认参数，如果需要更改，请在 make 程序中传参或通过编辑自己的 preset 实现。
