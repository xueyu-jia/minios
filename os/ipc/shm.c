...
static int newseg(int key, int id, int size) {
    ...
    if (key < 0 && key >= ids.perms_size) { disp_str("err key"); }
    ids.perms[id].state =
...
PUBLIC void *kern_shmat(int shmid, char *shmaddr, int shmflg) {
    ...
    if ((void *)phy_addr == NULL) {
        ...
        ids.perms[shmid].phy_address =
            (void *)pfn_to_phy(page_to_pfn(shm_page));
...
PUBLIC void *sys_shmmemcpy() {
    return do_shmmemcpy((void *)get_arg(1), (const void *)get_arg(2),
                        get_arg(3));
}
