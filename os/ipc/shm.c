...
PUBLIC struct spinlock lock_msg = {
    .locked = 0, .name = "ipc_lock", .cpu = 0xffffffff};
struct spinlock lock_shmmemcpy;
struct address_space shm_pages;

PUBLIC void init_shm() { init_mem_page(&shm_pages, MEMPAGE_SAVE); }
...
PUBLIC void *kern_shmat(int shmid, char *shmaddr, int shmflg) {
  ...
  // alignment(vir_addr);
  // lin_mapping_phy(vir_addr, phy_addr, p_proc_current->task.pid, PG_P | PG_USU
  // | PG_RWW, PG_P | PG_USU | PG_RWW);
...
PUBLIC void do_shmdt(char *shmaddr) { return kern_shmdt(shmaddr); }

PUBLIC void sys_shmdt() { return do_shmdt((char *)get_arg(1)); }
/*======================================================================*
                              shmctl
 *======================================================================*/
PUBLIC struct ipc_shm *kern_shmctl(int shmid, int cmd, struct ipc_shm *buf) {
  if (cmd == DELETE) {
    if (ids.perms[shmid].phy_address == NULL ||
        ids.perms[shmid].state == SHM_AVAILABLE) {
