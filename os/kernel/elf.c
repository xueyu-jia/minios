...
#include <kernel/vfs.h> //added by mingxuan 2019-5-23
 ...
 */
PUBLIC int read_elf(u32 fd, Elf32_Ehdr *Echo_Ehdr, Elf32_Phdr *Echo_Phdr,
                    Elf32_Shdr *Echo_Shdr) {
    int i;
    read_Ehdr(fd, Echo_Ehdr, 0);
    if ((*(u32 *)Echo_Ehdr->e_ident) != 0x464C457F) { return -1; }

    if (Echo_Ehdr->e_machine != EM_386) { return -1; }

    for (i = 0; i < Echo_Ehdr->e_phnum; i++) {
        read_Phdr(fd, &Echo_Phdr[i],
                  Echo_Ehdr->e_phoff + Echo_Ehdr->e_phentsize * i);
    }

    for (i = 0; i < Echo_Ehdr->e_shnum; i++) {
        read_Shdr(fd, &Echo_Shdr[i],
                  Echo_Ehdr->e_shoff + Echo_Ehdr->e_shentsize * i);
    }
    return 0;
}
