...
PUBLIC void init_process(
    PROCESS *proc, char name[32],
    enum proc_stat stat, int pid,
    int is_rt, int priority_or_nice
    )
{
 ...
 * @return {*}
 * @note
            cpu context switch to the "esp_save_context" whe scheduler switch this proc;
            cpu context switch to the "esp_save_int" whe cpu's rpl change to task level or kernel level (interrupt)
 */
