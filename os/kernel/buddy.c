...
/*u32 alloc_pages(buddy *bud, u32 order) //分配2^order页的页块
{
    ...
    if (bud->free_area[order].free_count > 0) //链表非空，直接取下一个页块
    {
        ...
        for (; i < bud->free_area[order].free_count; i++)
            ...
            bud->free_area[order].free_list[i] =
bud->free_area[order].free_list[i + 1];
    ...
    else //当前order页块链表为空
    {
        ...
        while (bud->free_area[tmp].free_count == 0) //向上搜寻非空的链表
        {
            ...
            if (tmp >= MAX_ORDER)
            {
                ...
                disp_color_str("buddy:alloc_pages Error,no available memory in
the buddy\n", 0x74);
        ...
        paddr = Split_buddy(bud, order, tmp);
//在上级链表中找到未分配的页块，将其拆分
    }
...
int free_pages(buddy *bud, u32 paddr, u32 order) //释放2^order页的页块
{
    ...
    if (tag == 0 || order == 10) //如果伙伴页块被分配，则直接将本页块挂在链表上
    {
        ...
        if (bud->free_area[order].free_count >= LIST_SIZE) //add by wang
2021.3.25
        {
...
static int list_insert(buddy *bud, u32 order, u32 paddr)
{
    ...
    for (j = count; j > i; j--)
    {
        ...
        bud->free_area[order].free_list[j] = bud->free_area[order].free_list[j -
1];
    }
...
//合并buddy
{
    ...
    for (i = 0; i < bud->free_area[order].free_count; i++)
    {
        ...
        if ((bud->free_area[order].free_list[i] / num_4K >> (order + 1)) ==
index) //找到伙伴页块
        {
            ...
            for (j = i; j < bud->free_area[order].free_count; j++)
            {
                ...
                bud->free_area[order].free_list[j] =
bud->free_area[order].free_list[j + 1];
            }
