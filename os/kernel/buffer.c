...
static inline buf_head *get_bh_lru() {
    ...
    if (bh == NULL) { disp_str("error: cant find buffer\n"); }
    return bh;
}
/*****************************************************************************
 *                                put_bh_lru
...
PUBLIC void rw_buffer(int iotype, buf_head *bh, int length) {
    ...
    if (kernel_initial == 1) { // init stage no sched
        rw_blocks(iotype, bh->dev, pos, length, proc2pid(p_proc_current),
                  bh->buffer);
    } else {
        rw_blocks_sched(iotype, bh->dev, pos, length, proc2pid(p_proc_current),
                        bh->buffer);
    }
    // disp_str("\nbuffer:");
    // disp_str(" blk:");
...
PRIVATE void try_sync_buffers(int background) {
    ...
    if ((buf_lru_cnt[BUFFER_DIRTY] > buf_dirty_nfrac * buf_block_cnt / 100) ||
        ((ticks > flush_tick + BUF_SYNC_CYCLE_TICK) &&
         buf_lru_cnt[BUFFER_DIRTY])) {
...
buf_head *getblk(u32 dev, u32 block) {
    ...
    if (!bh) {
        ...
        bh = get_bh_lru();
        if (bh->count) { disp_str("warning: still used blk\n"); }
        // 如果是脏块就立即写回硬盘
...
PUBLIC int blk_file_write(struct file_desc *file, unsigned int count,
                          const char *buf) {
    u64 pos = file->fd_pos;
    ...
    int chunk =
        min(rw_blk_max - rw_blk_min + 1, BLOCK_SIZE >> BLOCK_SIZE_SHIFT);
    int bytes_rw = 0;
...
PUBLIC int blk_file_read(struct file_desc *file, unsigned int count,
                         char *buf) {
    u64 pos = file->fd_pos;
    struct inode *inode = file->fd_dentry->d_inode;
    u64 pos_end = min(pos + count, inode->i_size);
