...
int fork() { return _syscall0(_NR_fork); }

int pthread_create(int* thread, void* attr, void* entry, void* arg) {
  return _syscall4(_NR_pthread_create, thread, attr, entry, arg);
}

void udisp_int(int arg) { _syscall1(_NR_udisp_int, arg); }

void udisp_str(char* arg) { _syscall1(_NR_udisp_str, arg); }
...
int close(int fd) { return _syscall1(_NR_close, fd); }
...
int unlink(const char* pathname) { return _syscall1(_NR_unlink, pathname); }

int creat(const char* pathname) { return _syscall1(_NR_creat, pathname); }
...
int wait() { return _syscall0(_NR_wait); }
...
void sigreturn(int ebp) { _syscall1(_NR_sigreturn, ebp); }

u32 total_mem_size() { return _syscall0(_NR_total_mem_size); }
...
int msgget(key_t key, int msgflg) { return _syscall2(_NR_msgget, key, msgflg); }
...
void test(int no) { _syscall1(_NR_test, no); }

u32 execvp(char* file, char* argv[]) {
  return _syscall2(_NR_execvp, file, argv);
}

u32 execv(char* path, char* argv[]) { return _syscall2(_NR_execv, path, argv); }

pthread_t pthread_self() { return _syscall0(_NR_pthread_self); }
...
int get_pid_byname(char* name) { return _syscall1(_NR_get_pid_byname, name); }
...
int umount(const char* target) { return _syscall1(_NR_umount, target); }

// int init_block_dev(int drive) {
// 	return _syscall1(_NR_init_block_dev, drive);
// }

void pthread_exit(void* retval) { _syscall1(_NR_pthread_exit, retval); }
...
void get_time(struct tm* time) { _syscall1(_NR_get_time, time); }

int stat(const char* pathname, struct stat* statbuf) {
  return _syscall2(_NR_stat, pathname, statbuf);
}

void nice(int val) { _syscall1(_NR_nice, val); }

void set_rt(int turn_rt) { _syscall1(_NR_set_rt, turn_rt); }

void rt_prio(int prio) { _syscall1(_NR_rt_prio, prio); }

void get_proc_msg(proc_msg* msg) { _syscall1(_NR_get_proc_msg, msg); }
