...
// static u32 do_kfree_static(u32 addr,u32 size);  //modified by mingxuan
// 2021-3-25
// static u32 phy_kfree_static(u32 addr, u32 size); //modified by mingxuan
// 2021-8-17
...
\n",0x74); return 0;
    ...
    if (i >= kmem.count) //否则分配一个4K的物理页面
    {
        ...
        //do_kfree(addr + size,num_4K - size);
//将分配完剩余的内存块挂在空闲分区表上
        //do_kfree_static(addr + size,num_4K - size); //modified by mingxuan
2021-3-25 phy_kfree_static(addr + size, num_4K - size); //modified by mingxuan
2021-8-17
    }
...
static u32 do_kfree_static(u32 addr,u32 size)   //modified by mingxuan 2021-3-25
{
    ...
    if(i > 0)//要释放的块的地址不是所有块中最低的，可能和前一块合并
    {
        ...
        if(kmem.kmem_table[i-1].addr + kmem.kmem_table[i-1].size == addr &&
kmem.kmem_table[i-1].addr / num_4K == pageid)
        {
            ...
            if(i < kmem.count)//第i-1块非最后一块，则可能和后一块合并
            {
                ...
                if(addr + size == kmem.kmem_table[i].addr &&
kmem.kmem_table[i].addr / num_4K == pageid)
                {
    ...
    if(i <
kmem.count)//i=0以及和前面的分区不连续都走这个分支，即和前面的块不连续或者前面无块，判断和后面的块是否连续
    {
...
kmem.kmem_table[i].addr / num_4K == pageid)
    ...
    if (i < kmem.count)
//i=0以及和前面的分区不连续都走这个分支，即和前面的块不连续或者前面无块，判断和后面的块是否连续
    {

        if (addr + size == kmem.kmem_table[i].addr && kmem.kmem_table[i].addr /
num_4K == pageid)
        {
