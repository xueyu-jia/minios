...
PUBLIC u32 do_kmalloc_4k() //无参数，从内核线性地址空间申请一页内存
{
        ...
        if (res == 0)
                disp_color_str("do_kmalloc_4k:alloc_pages Error,no memory",
0x74); return res;
}
*/
...
PUBLIC u32 do_malloc_4k() //无参数，从用户线性地址空间堆中申请一页内存
{
        ...
        if (res == 0)
                disp_color_str("do_malloc_4k:alloc_pages Error,no memory",
0x74); return res;
}
*/
...
//added by wang 2021.6.8
PUBLIC u32 do_kfree_over4k(u32 addr)  //有unsigned
int型参数addr，释放掉起始地址为addr的一段内存，大小由内存管理决定
{
...
*/
// PUBLIC int kern_mapping_4k(u32 AddrLin, u32 pid, u32 phyAddr, u32
// pte_attribute) { 	if (phyAddr != MAX_UNSIGNED_INT) {
// phy_mapping_4k(phyAddr);
// 	}
// 	return lin_mapping_phy(AddrLin, phyAddr, pid, PG_P | PG_USU | PG_RWW,
// pte_attribute);
// }
...
PUBLIC u32 do_malloc_4k() { return kern_malloc_4k(); }

// added by mingxuan 2021-8-11
PUBLIC u32 sys_malloc_4k() { return do_malloc_4k(); }
...
PUBLIC int do_free_4k(void *AddrLin) { return kern_free_4k(AddrLin); }

PUBLIC int sys_free_4k() { return do_free_4k((void *)get_arg(1)); }
...
PRIVATE struct address_space *get_mapping(LIN_MEMMAP *mmap,
                                          struct vmem_area *vma) {
  struct address_space *mapping = NULL;
  if (vma->file) {  // file mapped memory MEMPAGE_CACHED
    mapping = vma->file->fd_mapping;
  } else if (vma->flags &
             MAP_PRIVATE) {  // process private allocated memory MEMPAGE_AUTO
...
PRIVATE page *find_or_create_page(LIN_MEMMAP *mmap, struct vmem_area *vma,
                                  u32 pgoff) {
...
// require: pcb.memmap.vma_lock
PUBLIC void prepare_vma(PROCESS *p_proc, LIN_MEMMAP *mmap,
                        struct vmem_area *vma) {
  page *_page = NULL;
  u32 pgoff = vma->pgoff;
  // 页面权限
  ...
  for (u32 addr = vma->start; addr < vma->end; addr += PAGE_SIZE, pgoff++) {
    ...
    if ((vma->file) && (vma->flags & PROT_WRITE) &&
    ...
    lin_mapping_phy(addr, pfn_to_phy(page_to_pfn(_page)), proc2pid(p_proc),
                    PG_P | PG_USU | PG_RWW, pte_attr);
  }
}

PRIVATE void free_vma(PROCESS *p_proc, LIN_MEMMAP *mmap,
                      struct vmem_area *vma) {
...
// free vma [start, last)
PUBLIC void free_vmas(PROCESS *p_proc, LIN_MEMMAP *mmap,
                      struct vmem_area *start, struct vmem_area *last) {
  ...
  *new_mmap = *old_mmap;
  ...
  list_for_each(&old_mmap->vma_map, vma, vma_list) {
    ...
    *vm = *vma;
    ...
    u32 pte_attr =
        PG_P | PG_USU | ((vma->flags & PROT_WRITE) ? PG_RWW : PG_RWR);
    for (u32 addr = vma->start; addr < vma->end; addr += PAGE_SIZE) {
      ...
      if (phy) {
        ...
        lin_mapping_phy(addr, pfn_to_phy(page_to_pfn(_page)), proc2pid(p_child),
                        PG_P | PG_USU | PG_RWW, pte_attr);
#endif
      }
    }
...
void memmap_clear(PROCESS *p_proc) {
  ...
  free_vmas(p_proc, mmap,
            list_front(&mmap->vma_map, struct vmem_area, vma_list), NULL);
}
...
PUBLIC u32 kern_total_mem_size() {
  ...
  total_mem_size =
      kbud->current_mem_size + ubud->current_mem_size + kmem.current_mem_size;
