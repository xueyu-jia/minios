...
void memory_init() {
    ...
    for (; i < ALL_PAGES; i++) { mem_map[i].inbuddy = FALSE; }
        ...
        *vm = *vma;
        ...
        for (u32 addr = vma->start; addr < vma->end; addr += PAGE_SIZE) {
            ...
            if (phy) {
                ...
                lin_mapping_phy(addr, phy, proc2pid(p_parent),
                                PG_P | PG_USU | PG_RWW,
                                PG_P | PG_USU | PG_RWR); // set read only
                lin_mapping_phy(addr, phy, proc2pid(p_child),
                                PG_P | PG_USU | PG_RWW, PG_P | PG_USU | PG_RWR);
#else
                // 无写时复制，
                ...
                lin_mapping_phy(addr, pfn_to_phy(page_to_pfn(_page)),
                                proc2pid(p_child), PG_P | PG_USU | PG_RWW,
                                pte_attr);
#endif
            }
        }
    ...
    if (vma && vaddr >= vma->start) {
        ...
        if (_page) { // now new page is ready
            ...
            lin_mapping_phy(vaddr, pfn_to_phy(page_to_pfn(_page)),
                            proc2pid(p_proc_current), PG_P | PG_USU | PG_RWW,
                            attr);
            return 0;
        }
    }
