...
u32 vga_write_str(const char *str, u32 _disp_pos) {
    for (char *s = (char *)str; *s; ++s) { _disp_pos = vga_write_char(*s, WHITE, _disp_pos); }
	return _disp_pos;
}

u32 vga_write_colored_str(const char *str, u8 color, u32 _disp_pos) {
    for (char *s = (char *)str; *s; ++s) { _disp_pos = vga_write_char(*s, color, _disp_pos); }
	return _disp_pos;
}
...
void vga_copy(u32 dst, u32 src, size_t size) {
    memcpy(
        ...
        (const void*)K_PHY2LIN(V_MEM_BASE + (src << 1)),
        size << 1);
}
