...
#include "../include/spinlock.h" // added by ran
...
//added by ran
int ChangeDir(SUPER_BLOCK *psb, const char *dirname)
{
	DWORD parentCluster=0,off=0;
	CHAR fullpath[256]={0},parent[256]={0},name[256]={0};
	Record record;
...
STATE ReadFile(SUPER_BLOCK *psb, int fd,BYTE buf[], DWORD length)
{
	...
	//sector = (PBYTE)K_PHY2LIN(do_kmalloc(Bytes_Per_Sector*sizeof(BYTE)));	//modified by mingxuan 2021-3-25
	sector = (PBYTE)kern_kmalloc(Bytes_Per_Sector*sizeof(BYTE));	//modified by mingxuan 2021-3-25
	if(sector==NULL)
	{
		return SYSERROR;
	}
	...
	//sys_free(sector);
	//do_kfree(K_LIN2PHY(sector));	//modified by mingxuan 2021-3-25
	kern_kfree(sector);	//modified by mingxuan 2021-8-17
	//pfile->off = 0;
	return size;
}
...
STATE WriteFile(SUPER_BLOCK *psb, int fd,BYTE buf[],DWORD length)
{
	...
	{
		//return ACCESSDENIED;	//deleted by mingxuan 2021-8-31
		disp_str("file write access denied!\n");	//modified by mingxuan 2021-8-31
		return 0;
	}
	...
	//sector = (PBYTE)K_PHY2LIN(sys_kmalloc(Bytes_Per_Sector*sizeof(BYTE)));
	//sector = (PBYTE)K_PHY2LIN(do_kmalloc(Bytes_Per_Sector*sizeof(BYTE)));	//modified by mingxuan 2021-3-25
	sector = (PBYTE)kern_kmalloc(Bytes_Per_Sector*sizeof(BYTE));	//modified by mingxuan 2021-8-16
	if(sector==NULL)
	{
		//return SYSERROR;	//deleted by mingxuan 2021-8-31
		disp_str("file write system error!\n");	//modified by mingxuan 2021-8-31
		return 0;
	}
	...
	if(pfile->start==0)//此文件是个空文件原来没有分配簇
	{
		...
		if(state!=OK)
		{
			...
			//return state;//虚拟磁盘空间不足 	//deleted by mingxuan 2021-8-31
			disp_str("No enough free space on disk!\n");	//modified by mingxuan 2021-8-31
			return 0;
		}
	}else{
	...
	release(plock); //added by ran
	//return OK;
	return length;	//modified by mingxuan 2021-8-29
}
...
int read_adapter(int fd,BYTE buf[], DWORD length)
{
	...
	return ReadFile(pvfs->sb, fd, buf, length);
}
