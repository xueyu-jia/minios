...
PUBLIC int kern_pthread_create(pthread_t *thread, pthread_attr_t *attr,
                               void *entry,
                               void *arg)  // modified by mingxuan 2021-8-14
{
  PROCESS *p_child;
  ...
  if (attr == NULL) {
    ...
    true_attr.detachstate =
        PTHREAD_CREATE_JOINABLE;  // modified by dongzhangqi 2023.5.4
                                  // Detachstate的默认设置应为joinable，允许调用pthread_join
    ...
    true_attr.stackaddr =
        (void *)(p_parent->task.memmap.stack_child_limit + 0x4000);
  ...
  if (0 == p_child) {
    ...
    //*((u32*)(p_reg + EIPREG - P_STACKTOP)) = p_child->task.regs.eip;	//added
    // by xw, 17/12/11
    ...
    // p_reg = (char*)(p_child + 1);	//added by xw, 17/12/11
    //*((u32*)(p_reg + EAXREG - P_STACKTOP)) = p_child->task.regs.eax;	//added
    // by xw, 17/12/11
    ...
    *((u32 *)reg_esp) = (u32)arg;
    ...
    // anything child need is prepared now, set its state to ready. added by xw,
    // 17/12/11
...
PUBLIC int sys_pthread_create() {
  return do_pthread_create((int *)get_arg(1), (void *)get_arg(2),
                           (void *)get_arg(3), (void *)get_arg(4));
}
...
PRIVATE int pthread_stack_init(PROCESS *p_child, PROCESS *p_parent,
  ...
  // {
  // 	//disp_str("#");
  // 	//lin_mapping_phy(addr_lin, MAX_UNSIGNED_INT, p_child->task.pid, PG_P  |
  // PG_USU | PG_RWW,PG_P  | PG_USU | PG_RWW);
  // 	ker_umalloc_4k(addr_lin,p_child->task.pid,PG_P  | PG_USU | PG_RWW);
  // //edited by wang 2021.8.29
  // }
  kern_mmap(p_child, NULL, p_child->task.memmap.stack_lin_limit,
            attr->stacksize, PROT_READ | PROT_WRITE, MAP_PRIVATE, 0);

  // p_child->task.regs.esp = p_child->task.memmap.stack_lin_base;
  // //调整esp
  // p_reg = (char*)(p_child + 1);	//added by xw, 17/12/11
  //*((u32*)(p_reg + ESPREG - P_STACKTOP)) = p_child->task.regs.esp;	//added
  // by xw, 17/12/11
  ...
  // p_child->task.regs.ebp = p_child->task.memmap.stack_lin_base;
  // //调整ebp
  // p_reg = (char*)(p_child + 1);	//added by xw, 17/12/11
  //*((u32*)(p_reg + EBPREG - P_STACKTOP)) = p_child->task.regs.ebp;	//added
  // by xw, 17/12/11
  // *((u32*)(p_reg + EBPREG - P_STACKTOP)) =
  // p_child->task.memmap.stack_lin_base;
...
PUBLIC pthread_t kern_pthread_self() { return p_proc_current->task.pthread_id; }
PUBLIC pthread_t do_pthread_self() { return kern_pthread_self(); }
PUBLIC pthread_t sys_pthread_self() { return do_pthread_self(); }
...
PUBLIC void do_pthread_exit(void *retval) { return kern_pthread_exit(retval); }

PUBLIC void sys_pthread_exit() { return do_pthread_exit((void *)get_arg(1)); }
