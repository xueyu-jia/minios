        ...
        if (hd_infos[i].open_cnt > 0) {
            for (int j = 0; j < 16; j++) {
                if (hd_infos[i].part[j].size > 0) {
                    int major = i, minor = j;
                    UNUSED(major);
                    register_device(MAKE_DEV(DEV_HD_BASE + i, minor),
                                    DEV_BLOCK_TYPE, &blk_file_ops);
                }
            }
        }
    }
...
int init_char_dev() {
    ...
    for (int i = 0; i < NR_CONSOLES; ++i) {
        ...
        register_device(MAKE_DEV(DEV_CHAR_TTY, i), DEV_CHAR_TYPE,
                        &tty_file_ops);
    }
    return 0;
}
...
PRIVATE int page_filemap(page* target, struct address_space* file_mapping,
    ...
    if (create == 0) {
        ...
        if (nr == 0) { disp_str("error: over limit blk nr\n"); }
    }
...
int generic_file_write(struct file_desc* file, unsigned int count,
    ...
    if (cnt != count) { disp_str("write cnt wrong?"); }
    return cnt;
}
