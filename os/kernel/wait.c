...
PRIVATE PROCESS* try_get_zombie_child(u32 pid) {
  ...
  for (int i = 0; i < pcb->task.tree_info.child_p_num; i++) {
    PROCESS* exit_child =
        (PROCESS*)pid2proc(pcb->task.tree_info.child_process[i]);
...
PUBLIC int do_wait(
    int* status)  // wait返回的为子进程pid,子进程退出状态通过*status传递
                  // modified by dongzhangqi 2023-4-20
{
  return kern_wait(status);
}

PUBLIC int
sys_wait()  // wait返回的为子进程pid modified by dongzhangqi 2023-4-20
{
