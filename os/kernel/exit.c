...
PUBLIC void kern_exit(int exit_code) {
  ...
  assert(exit_pcb->task.tree_info.ppid >= 0 &&
         exit_pcb->task.tree_info.ppid < NR_PCBS);
...
PUBLIC void do_exit(int status) { kern_exit(status); }

// added by mingxuan 2021-8-13
PUBLIC void sys_exit() { do_exit(get_arg(1)); }
...
PRIVATE int transfer_child_proc(u32 src_pid, u32 dst_pid) {
  ...
  for (int i = 0; i < src_pcb->task.tree_info.child_p_num; i++) {
    ...
    dst_pcb->task.tree_info.child_process[index] =
        src_pcb->task.tree_info.child_process[i];
    PROCESS* son_pcb =
        (PROCESS*)pid2proc(src_pcb->task.tree_info.child_process[i]);
