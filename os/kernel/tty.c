...
*/
    ...
    switch (raw_code) {
      ...
      case UP:
        ...
        //     outb(CRTC_DATA_REG, (
        //     (80*(p_tty->console->current_line+real_line)) >> 8) & 0xFF);
        //     outb(CRTC_ADDR_REG, START_ADDR_L);
        //     outb(CRTC_DATA_REG, (80*(p_tty->console->current_line+real_line))
        //     & 0xFF); enable_int( );
        // }
...
PUBLIC int tty_file_write(struct file_desc* file, unsigned int count,
                          const char* buf) {
...
PUBLIC int tty_file_read(struct file_desc* file, unsigned int count,
                         char* buf) {
  int dev = file->fd_dentry->d_inode->i_b_cdev;
  int nr_tty = MINOR(dev);
  if (MAJOR(dev) != DEV_CHAR_TTY) {
