...
PRIVATE PROCESS* alloc_task_PCB()
{
	PROCESS* p;
...
PRIVATE PROCESS* alloc_init_PCB()
{
	PROCESS* p;
	// p = proc_table+NR_K_PCBS;//跳过前NR_K_PCBS个
	p = &proc_table[16];
	if(p->task.stat == FREE) return p;
    disp_str("alloc init pcb error");
	return NULL;
}
...
PUBLIC int kthread_create(char *name, void *func, int is_rt, int priority)
{
    ...
    init_process(p_proc, name, READY, p_proc->task.pid, is_rt, priority);
