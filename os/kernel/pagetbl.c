 ...
 *======================================================================*/
PRIVATE inline u32 get_pde_index(u32 AddrLin) { // 由 线性地址 得到
                                                // 页目录项编号
 ...
 *======================================================================*/
PRIVATE inline u32 get_pte_index(u32 AddrLin) { // 由 线性地址 得到 页表项编号
    return (((AddrLin) & 0x003FFFFF) >>
            12); // 中间10位A21~A12,0x3FFFFF = 0000 0000 0011 1111 1111 1111
                 // 1111 1111
}
 ...
 *======================================================================*/
PRIVATE u32 get_pde_phy_addr(u32 pid) {  // 获取页目录物理地址
 ...
 *======================================================================*/
PRIVATE inline u32 get_pte_phy_addr(
    u32 PageDirPhyAddr,      // 页目录物理地址		//edit by visual
                             //  2016.5.19
    u32 AddrLin)             // 线性地址
{                            // 获取该线性地址所属页表的物理地址
...
/*======================================================================*
                          get_page_phy_addr	add by visual 2016.5.9  refact
 20240405 jiangfeng
 *======================================================================*/
PRIVATE inline u32 get_page_phy_addr_nopid(
    u32 PageTblPhyAddr, // 页表物理地址				//edit
                        // by visual
                        //  2016.5.19
    u32 AddrLin)        // 线性地址
{                       // 获取该线性地址对应的物理页物理地址
...
{ // 判断 该线性地址 有没有 对应的 物理页
    if ((0x00000001 & (*((u32 *)K_PHY2LIN(PageTblPhyAddr) +
                         get_pte_index(AddrLin)))) == 0) { // 标志位为0，不存在
        return 0;
    } else {
        return 1;
    }
 ...
 *======================================================================*/
PUBLIC int init_proc_page(u32 pid) { // 页表初始化函数

    u32 pde_addr_phy_temp;

    // pde_addr_phy_temp = do_kmalloc_4k();//为页目录申请一页
    pde_addr_phy_temp =
        phy_kmalloc_4k(); // 为页目录申请一页	//modified by mingxuan 2021-8-16
    ...
    memcpy((void *)(K_PHY2LIN(pde_addr_phy_temp) + kernel_pde_offset),
           (void *)(K_PHY2LIN(kernel_pde_phy) + kernel_pde_offset),
           num_4K - kernel_pde_offset);
...
PUBLIC int init_kernel_page() {
    ...
    for (AddrLin = KernelLinBase; AddrLin < KernelLinMapLimit;
         AddrLin += num_4M) {
        phy_addr = (u32)
            phy_kmalloc_4k(); // 为页表申请一页	//modified by mingxuan 2021-8-16

        memset((void *)K_PHY2LIN(phy_addr), 0,
               num_4K); // add by visual 2016.5.26
    ...
    for (AddrLin = KernelLinBase, phy_addr = 0;
         AddrLin < KernelLinBase + kernel_size;
         AddrLin += num_4K,
        phy_addr +=
         num_4K) { // 只初始化内核部分，3G后的线性地址映射到物理地址开始处
        int err_temp = lin_mapping_phy_nopid(
            AddrLin,  // 线性地址					//add by visual
                      //  2016.5.9
            phy_addr, // 物理地址
            kernel_pde_addr_phy,
            PG_P | PG_USS | PG_RWW,  // 页目录的属性位（系统权限）
...
{                 // 缺页中断处理函数
    ...
    if (0 == pte_exist(pde_addr_phy_temp, cr2) ||
        0 == phy_exist(pte_addr_phy_temp, cr2)) {
        ...
        // 检查是否为合法物理地址?
        if ((phy_addr) <
            ((big_kernel) ? KUWALL2 : KUWALL1)) { // 错误的物理页，这不对吧,
            goto fatal;
        } else {
            // 检查权限
            if ((err_code & 2) &&
                (!(attr & PG_RWW))) { // 缺页原因为写入，页面无写权限
                fault_flag |= FAULT_WRITE;
            }
        }
    }
    if (cr2 >= KernelLinBase) { goto fatal; }
#ifdef OPT_MMU_COW
...
// added by mingxuan 2021-8-25
PUBLIC int lin_mapping_phy_nopid(
    u32 AddrLin,  // 线性地址
    ...
    u32 pte_Attribute) // 页表中的属性位
{
    ...
    if (0 == pte_exist(pde_addr_phy,
        ...
        pte_addr_phy =
            (*((u32 *)K_PHY2LIN(pde_addr_phy) + get_pde_index(AddrLin))) &
            0xFFFFF000;
    }
    ...
    {                                 // 由函数申请内存
        if (0 == phy_exist(pte_addr_phy,
            ...
            if (AddrLin >= K_PHY2LIN(0)) {
                ...
                phy_addr =
                    phy_kmalloc_4k(); // 从内核物理地址申请一页	//modified by
                                      //  mingxuan 2021-8-16
            } else {
                // disp_str("%");
    ...
    u32 pte_Attribute) // 页表中的属性位
{
    ...
    return lin_mapping_phy_nopid(AddrLin, phy_addr, pde_addr_phy, pde_Attribute,
                                 pte_Attribute);
}
...
PUBLIC int kmapping_phy(u32 phy_addr) {
    ...
    if (phy_addr >=
        K_LIN2PHY(KernelLinMapBase)) { // 高端地址，3G+phy_addr无法直接访问
        ...
        int index = 0;
        while (index < KernelLinMapMaxPage && kmapping_pages[index] != 0)
            index++;
        ...
        lin_addr =
            K_PHY2LIN(phy_addr); // 用户页物理地址，默认没有内核页表，需要写页表
    } else {
        release(&kmap_lock);
        return K_PHY2LIN(phy_addr); // 无需映射，可直接访问
    }
    lin_mapping_phy_nopid(lin_addr, phy_addr, kernel_pde_phy,
...
PUBLIC int kunmapping_phy(u32 phy_addr) {
    ...
    if (phy_addr >= K_LIN2PHY(KernelLinMapBase)) {
        ...
        int index = 0;
        while (index < KernelLinMapMaxPage && kmapping_pages[index] != phy_addr)
            index++;
...
// 	//释放栈
// 	/*
// 	if(type == MEMMAP_STACK)
// //栈段是从高低址向低地址生长的，释放时与其他不同
// 	{
// 		//特别注意，limit的物理地址是取不到的，因为之前没有对limit所在的线性地址做映射,如何处理还需要再思考,
// mingxuan 2021-1-7 		u32 limit_phy_addr = get_page_phy_addr(pid, base
// - num_4K)
// - num_4K;//栈空间必须是连续的，不然会出错 		for (addr_lin = base -
// num_4K; addr_lin > limit; addr_lin -= num_4K)
// 		{
...
// 	{
// 		//特别注意，limit的物理地址是取不到的，因为之前没有对limit所在的线性地址做映射,如何处理还需要再思考,
// mingxuan 2021-1-7 		for (addr_lin = limit; addr_lin <
// UPPER_BOUND_4K(base); addr_lin += num_4K)
// 		{
// 			free_phypage(pid, addr_lin);
// 		}
// 	}
...
// 	else //释放其他段
// 	{
// 		for (addr_lin = base; addr_lin < UPPER_BOUND_4K(limit); addr_lin
// += num_4K)
// 		{
// 			free_phypage(pid, addr_lin);
// 		}
...
PUBLIC void free_seg_pagetbl(u32 pid, u8 type) {
    ...
    for (addr_lin = low; addr_lin < high; addr_lin += num_4M) {
        if (1 ==
            pte_exist(get_pde_phy_addr(pid), addr_lin)) // 解决重复释放的问题
        {
...
*                          update_heap_ptr		added by mingxuan
2021-3-25
*
      更新核内堆指针，使用户态下和核心态下堆空间保持一致
*======================================================================*/
...
u32 get_pte(u32 addrLin) {
    u32 pte_phy_addr =
        get_pte_phy_addr(get_pde_phy_addr(p_proc_current->task.pid), addrLin);
