...
// 初始化中断帧
void init_int_frame(STACK_FRAME *frame, u32 cs, u32 ds, u32 es, u32 fs, u32 ss,
                    u32 gs, u32 eflags, u32 esp, u32 eip) {
    frame->cs = cs;
    frame->ds = ds;
    frame->es = es;
    frame->fs = fs;
...
void alloc_ldt(int pid, cpu_context *context, u32 rpl) {
    ...
    memcpy(&context->ldts[0], &gdt[SELECTOR_KERNEL_CS >> 3],
           sizeof(DESCRIPTOR));
    context->ldts[0].attr1 = DA_C | privilege << 5;
    memcpy(&context->ldts[1], &gdt[SELECTOR_KERNEL_DS >> 3],
           sizeof(DESCRIPTOR));
    context->ldts[1].attr1 = DA_DRW | privilege << 5;
}
void init_cpu_context(cpu_context *context, int pid, u32 int_eip, u32 int_esp,
                      u32 context_eip, u32 rpl) {
    ...
    init_int_frame(context->esp_save_int, common_cs | rpl, common_ds | rpl,
                   common_es | rpl, common_fs | rpl, common_ss | rpl,
                   common_gs | rpl, eflags, (u32)int_esp, (u32)int_eip);
