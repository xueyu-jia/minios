...
PRIVATE int exec_load(u32 fd, const Elf32_Ehdr* Echo_Ehdr,
                      const Elf32_Phdr* Echo_Phdr);
PRIVATE int exec_pcb_init(const char* path);
PRIVATE int exec_count_args(char* const* pstr, u32* count);
PRIVATE int exec_copy_args(char* const* pstr, char** parr, char* dst,
                           u32 offset);
PRIVATE int exec_replace_argv_and_envp(const char* path, char* const* argv,
                                       char* const* envp);
...
PUBLIC u32 kern_execve(const char* path, char* const* argv, char* const* envp) {
  ...
  void* entry_point =
      exec_read_and_load(fd);  //读取并加载可执行文件的内容，并关闭文件
  kern_vfs_close(fd);
  if (entry_point == NULL) {
  ...
  err_temp = do_mmap(p_proc_current->task.memmap.stack_lin_limit,
                     p_proc_current->task.memmap.stack_lin_base -
...
PRIVATE int exec_replace_argv_and_envp(const char* path, char* const* argv,
  ...
  space +=
      (count + 5) * 4;  // argc + argv + envp + 1(args end 0) + 1(env end 0)
  // 补充: 参数数量不能超过最大值
  ...
  *((char**)(tmp_buf + 8)) = (char*)envs_base_in_tmp + offset;
  ...
  exec_copy_args(envp, envs_base_in_tmp, args_raw_base_in_tmp + env_space,
                 offset);
...
PRIVATE void* exec_read_and_load(int fd) {
  ...
  Elf32_Phdr* elf_proghs =
      (Elf32_Phdr*)kern_kmalloc(sizeof(Elf32_Phdr) * elf_header->e_phnum);
  assert(elf_proghs != NULL);

  for (int i = 0; i < elf_header->e_phnum; i++) {
    u32 offset = elf_header->e_phoff +
                 i * sizeof(Elf32_Phdr);  //计算当前程序头在文件中的偏移量。
    read_Phdr(fd, elf_proghs + i,
...
PRIVATE int exec_load(u32 fd, const Elf32_Ehdr* Echo_Ehdr,
  ...
  for (ph_num = 0, phdr = Echo_Phdr; ph_num < Echo_Ehdr->e_phnum;
    ...
    if (phdr->p_type == ELF_LOAD) {
      ...
      if (file_page_size) {
        addr = do_mmap(mem_start, file_page_size, prot, MAP_PRIVATE | MAP_FIXED,
                       fd, phdr->p_offset - in_page_offset);
        if (addr != mem_start) {
      ...
      if (mem_size > file_size) {  // bss
        if (mem_size > file_page_size) {  // bss 超过elf映射的页，额外分配匿名页
          addr =
              do_mmap(mem_start + file_page_size, mem_size - file_page_size,
...
PRIVATE int exec_pcb_init(const char* proc_name) {
  ...
  init_user_cpu_context(&p_proc_current->task.context,
  ...
  // //当前父进程 p_proc_current->task.tree_info.child_p_num = 0;
  // //子进程数量
...
PUBLIC u32 sys_execve() {
  return do_execve((const char*)get_arg(1), (char* const*)get_arg(2),
                   (char* const*)get_arg(3));
}
