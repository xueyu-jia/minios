...
PUBLIC int hd_rdwt_base(int drive, int type, u32 sect_nr, u32 count,
                        void *buf) {
...
PRIVATE void hd_rdwt_real(RWInfo *p) { hd_rdwt(p->msg); }
...
PUBLIC void init_hd_queue(HDQueue *hdq) { hdq->front = hdq->rear = NULL; }
...
PRIVATE int partition_get_fstype(int drive, int start_sect) {
  for (int type = 1; type < NR_FS_TYPE; type++) {
    if (fstype_table[type].identify) {
      if (fstype_table[type].identify(drive, (u32)start_sect) == 1) {
        return type;
      }
    }
  }
...
PRIVATE void partition(int device, int style) {
  ...
  if (style == P_PRIMARY) {
    ...
    for (i = 0; i < NR_PART_PER_DRIVE; i++) { /* 0~3 */
      ...
      hdi->part[dev_nr].fs_type =
          partition_get_fstype(drive, hdi->part[dev_nr].base);
      // added by mingxuan 2020-10-27
      // struct fs_flags fs_flags_real;
      // struct fs_flags *fs_flags_buf = &fs_flags_real;
      // get_fs_flags(drive, hdi->part[dev_nr].base+BLOCK_SIZE/SECTOR_SIZE,
      // fs_flags_buf); //hdi->primary[dev_nr].base + 1 beacause of orange and
      // fat32 is in 2nd sector, mingxuan if(fs_flags_buf->orange_flag == 0x11)
...
PRIVATE int waitfor(int mask, int val, int timeout) {
  ...
  */
  ...
  while (((kern_get_ticks() - t) * 1000 / HZ) <
         timeout) {  // modified by mingxuan 2021-8-14
...
PRIVATE int SATA_rdwt_sects(int drive, int type, u64 sect_nr, u32 count) {
  ...
  cmdheader->w = (type == DEV_READ)
                     ? 0
                     : 1;  // 0:device to host,read ;1:host to device,write
  cmdheader->c = 0;
  ...
  cmdfis->command =
      (type == DEV_READ) ? ATA_CMD_READ_DMA_EX : ATA_CMD_WRITE_DMA_EX;
...
// added by xw, 18/8/27
// int rw_sector_sched(int io_type, int dev, u64 pos, int bytes, int proc_nr,
// void *buf)
// {
...
int rw_blocks(int io_type, int dev, u64 pos, int bytes, int proc_nr,
              void *buf) {
  MESSAGE *driver_msg = (MESSAGE *)kern_kmalloc(sizeof(MESSAGE));
  driver_msg->type = io_type;
  // driver_msg.DEVICE	= MINOR(dev);
...
// add by sundong 2023.5.26
int rw_blocks_sched(int io_type, int dev, u64 pos, int bytes, int proc_nr,
                    void *buf) {
...
// added by mingxuan 2020-10-27
// refact 20240328
PUBLIC int orangefs_identify(int drive, u32 _sect_nr) {
  struct fs_flags fs_flags_real;
  hd_rdwt_base(drive, DEV_READ, (u32)(_sect_nr + 8), sizeof(struct fs_flags),
               &fs_flags_real);
