...
PRIVATE void kmem_cache_init(kmem_cache_t *cache, const char *name,
                             u32 objorder);
...
int kfree(u32 object) {
    ...
    while (!flag) { // 对象所在的链表可能还没有添加到新的链表，循环查找
        for (int i = PARTIAL; i <= FULL && !flag;
             i++) {                          // 依次查找partial链和full链
...
kmem_slab_t *kmem_cache_new_slab(kmem_cache_t *cache) {
    ...
    slab->objects = (u32)ptr_offset(
        slab->bitmap, (cache->bitmap_length) * sizeof(bitmap_entry_t));
    slab->objects =
        ROUNDUP(slab->objects, cache->objorder); // 按照size对齐	lirong
    slab->type = EMPTY;
...
PRIVATE void kmem_cache_init(kmem_cache_t *cache, const char *name,
    ...
    cache->slab_count[EMPTY] = cache->slab_count[PARTIAL] =
        cache->slab_count[FULL] = 0; // 统计slab数目
...
PRIVATE void *kmem_cache_alloc_obj(kmem_cache_t *cache) {
    ...
    if (cache->slab_count[PARTIAL] > 0) {
        ...
        flag = 1;                      // 表示分配对象成功
        if (slabCheckfull(
                slab)) { // 判断此时page类型，page所有对象是否全部被分配
...
void printInfo(kmem_cache_t *cache) {
    ...
    disp_str(cache->name);
    disp_str("\nObject size: ");
    disp_int(cache->objsize);
    disp_str("\nObject num per Slab: ");
    disp_int(cache->obj_per_slab);
    disp_str("\nslab info\n");
    disp_str("Number of EMPTY    slabs: ");
    disp_int(cache->slab_count[EMPTY]);
    disp_str("\nNumber of PARTIAL  slabs: ");
    disp_int(cache->slab_count[PARTIAL]);
    disp_str("\nNumber of FULL     slabs: ");
    disp_int(cache->slab_count[FULL]);
    disp_str("\nobject info\n");
    disp_str("Number of all  objects: ");
    disp_int(all_object);
    disp_str("\nNumber of used objects: ");
    disp_int(used_object);
    disp_str("\nNumber of free objects: ");
    disp_int(free_object);
    // disp_str("\nUsed space: %.1f%%", usage);
    disp_str("\n\n");
}
