...
PRIVATE struct inode* alloc_inode_locked(struct super_block* sb) {
    ...
    const int inode_impl_size =
        sizeof(struct inode) +
        fstype_table[sb->fs_type].fs_size_info.inode_size;
...
PRIVATE const char* strip_dir_path(const char* path, char* dir) {
    ...
    UNUSED(flag);
    const char *p = path + len - 1, *file_name = path,
               *file_end = path + len - 1;
    UNUSED(file_end);
    while (p != path && (*p) == '/') p--;
    file_end = p;
...
PRIVATE void init_special_inode(struct inode* inode, int type, int mode,
    ...
    switch (type) {
        ...
        case I_BLOCK_SPECIAL:
            inode->i_size =
                hd_infos[MAJOR(dev) - DEV_HD_BASE].part[MINOR(dev)].size
                << SECTOR_SIZE_SHIFT;
...
PUBLIC void register_fs_types() {
    ...
    register_fs_type("orangefs", ORANGE_TYPE, &orange_file_ops,
                     &orange_inode_ops, NULL, &orange_sb_ops,
                     orangefs_identify);
...
struct dirent* kern_vfs_readdir(DIR* dirp) {
    if (!dirp->init) {
        ...
        if (inode->i_fop && inode->i_fop->readdir) {
            dirp->total = inode->i_fop->readdir(
                dirp->file, num_4K - sizeof(DIR), data_start);
        }
        dirp->init = 1;
        release(&inode->lock);
    }
    ...
    if (dirp->pos < dirp->total) {
        ...
        if (!strcmp(ent->d_name, "..")) { // check parent needed for mount point
            ...
            struct dentry* found =
                _do_lookup(dirp->file->fd_dentry, ent->d_name, 1);
            if (found) {
