...
PRIVATE void vfs_get_path(struct dentry* dir, char* buf, int size) {
  ...
  *(--p) = 0;
  ...
  while (dir && dir != vfs_root) {
    while (dir != vfs_root && dir->d_vfsmount &&
           dir->d_vfsmount->mnt_root == dir) {
...
PRIVATE struct dentry* _do_lookup(struct dentry* dir, const char* name,
                                  int release_base) {
...
PRIVATE void init_special_inode(struct inode* inode, int type, int mode,
                                int dev) {
...
PRIVATE struct dentry* vfs_create(struct inode* dir, const char* file_name,
                                  int type, int mode, int dev) {
  struct inode_operations* i_ops = NULL;
  struct dentry* dentry = NULL;
  struct inode* inode = NULL;
  int state = -1;
...
// 挂载块设备核心实现
// 返回文件系统分区实例的根目录项
PRIVATE struct dentry* vfs_mount_dev(int dev, u32 fstype, const char* dev_name,
                                     struct dentry* mnt) {
...
PUBLIC void register_fs_types() {
  register_fs_type("devfs", DEV_FS_TYPE, &devfs_file_ops, &devfs_inode_ops,
                   NULL, &devfs_sb_ops, NULL);

  register_fs_type("orangefs", ORANGE_TYPE, &orange_file_ops, &orange_inode_ops,
                   NULL, &orange_sb_ops, orangefs_identify);

  register_fs_type("fat32", FAT32_TYPE, &fat32_file_ops, &fat32_inode_ops,
                   &fat32_dentry_ops, &fat32_sb_ops, fat32_identify);
}
...
PRIVATE struct file_desc* vfs_file_open(const char* path, int flags, int mode) {
  ...
  if (!dentry && (flags & O_CREAT)) {
    // create regular file
    dentry = vfs_create(dir->d_inode, file_name, I_REGULAR,
                        mode & (~I_TYPE_MASK), 0);
...
PUBLIC int kern_vfs_mknod(const char* path, int mode, int dev) {
  ...
  dentry = _do_lookup(dir, file_name, 0);
  if (dentry) {
    init_special_inode(dentry->d_inode, mode & I_TYPE_MASK,
                       mode & (~I_TYPE_MASK), dev);
  } else {
    dentry = vfs_create(dir->d_inode, file_name, mode & I_TYPE_MASK,
                        mode & (~I_TYPE_MASK), dev);
    if (!dentry) {
      release(&dir->lock);
      return -1;
...
PUBLIC int kern_vfs_mkdir(const char* path, int mode) {
  ...
  dentry = vfs_create(dir->d_inode, file_name, I_DIRECTORY,
                      mode & (~I_TYPE_MASK), 0);
  if (!dentry) {
    vfs_put_dentry(dir);
    return -1;
...
PUBLIC int kern_vfs_rmdir(const char* path) {
  ...
  if (dentry == vfs_root ||
      (dentry->d_vfsmount &&
       dentry->d_vfsmount->mnt_root == dentry)) {  // 不允许删除挂载点
    goto err;
  }
  if (inode->i_type != I_DIRECTORY) {
    goto err;
...
struct dirent* kern_vfs_readdir(DIR* dirp) {
  if (!dirp->init) {
    ...
    if (inode->i_fop && inode->i_fop->readdir) {
      dirp->total =
          inode->i_fop->readdir(dirp->file, num_4K - sizeof(DIR), data_start);
    }
    dirp->init = 1;
    release(&inode->lock);
  }
...
PUBLIC int do_vclose(int fd) { return kern_vfs_close(fd); }
...
PUBLIC int do_vunlink(const char* path) { return kern_vfs_unlink(path); }

PUBLIC int do_vcreat(const char* filepath) {
  return kern_vfs_creat(filepath, I_RWX);
}

PUBLIC int do_vopendir(const char* path) { return (int)kern_vfs_opendir(path); }

PUBLIC int do_vclosedir(DIR* dirp) { return kern_vfs_closedir(dirp); }

PUBLIC int do_vmkdir(const char* path, int mode) {
  return kern_vfs_mkdir(path, mode);
}

PUBLIC int do_vrmdir(const char* path) { return kern_vfs_rmdir(path); }

PUBLIC struct dirent* do_vreaddir(DIR* dirp) { return kern_vfs_readdir(dirp); }

PUBLIC int do_vchdir(const char* path) { return kern_vfs_chdir(path); }
...
PUBLIC int sys_close() { return do_vclose(get_arg(1)); }
...
PUBLIC int sys_lseek() { return do_vlseek(get_arg(1), get_arg(2), get_arg(3)); }

PUBLIC int sys_unlink() { return do_vunlink((const char*)get_arg(1)); }

PUBLIC int sys_creat() { return do_vcreat((const char*)get_arg(1)); }

PUBLIC int sys_closedir() { return do_vclosedir((DIR*)get_arg(1)); }

PUBLIC int sys_opendir() { return do_vopendir((const char*)get_arg(1)); }

PUBLIC int sys_mkdir() {
  return do_vmkdir((const char*)get_arg(1), get_arg(2));
}

PUBLIC int sys_rmdir() { return do_vrmdir((const char*)get_arg(1)); }

PUBLIC int sys_readdir() { return (int)do_vreaddir((DIR*)get_arg(1)); }

// added by ran
PUBLIC int sys_chdir() { return do_vchdir((const char*)get_arg(1)); }
