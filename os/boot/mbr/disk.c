...
static bool find_act_part_in_ext(void *buffer, u32 ext_start_sector) {
  ...
  // lprintf("ext active flag %d lba start
  ...
  */
  ...
  for (int i = 0; i < PART_NUM; i++)  // 只有4个分区，每个分区16个字节
  {
    ...
    if (p_mbr_entry->status == ACT_PART_FLAG)  // 活跃分区
    {
      /*       u32 temp = 0;
            start = start + 12;  //
         分区的起始扇区号在8字节处，长度为4个字节，从后往前读 temp = temp +
         (u32)(*start); for (int j = 0; j < 4; j++)  // 读取4个字节
            {
              start = start - 1;
...
//只读一个sect
int readsect(void *dst, u32 offset) { return readsects(dst, offset, 1); }
//一次读多个sector
