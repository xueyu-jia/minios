...
PUBLIC int AHCI_init()  //遍历pci设备，找到AHCI  by qianglong	2022.5.17
{
    ...
    for (dev = 0; dev <= MAXDEV; dev++) {
      for (fun = 0; fun <= MAXFUN; fun++) {
        ...
        if ((in_dword(PCI_CONFIG_DATA) | 0xffff) !=
          ...
          out_dword(0xcf8, mk_pci_add(bus, dev, fun,
                                      2));  // 2号寄存器为classcode,设备类别
  ...
  /* 	//将AHCI基地址映射到线性地址
     ...
     by visual 2016.5.26 PG_P | PG_USS | PG_RWW); //页表的属性位（系统权限）
  ...
  /* 	int hd_service_pid = sys_get_pid_byname("hd_service");
     ...
     //add by visual 2016.5.9 ahci_info[0].ABAR, //物理地址 hd_service_pid, PG_P
     | PG_USS | PG_RWW,  //页目录的属性位（系统权限）			//edit
     by visual 2016.5.26 PG_P | PG_USS | PG_RWW); //页表的属性位（系统权限）
          ...
          err_temp = lin_mapping_phy(	ahci_info[0].ABAR,  //线性地址
     //add by visual 2016.5.9 ahci_info[0].ABAR, //物理地址 initial_pid, PG_P |
     PG_USS | PG_RWW,  //页目录的属性位（系统权限）			//edit
     by visual 2016.5.26 PG_P | PG_USS | PG_RWW); //页表的属性位（系统权限）

          if (err_temp != 0)
          {
                  disp_color_str("kernel page Error:lin_mapping_phy_ahci",
     0x74); return FALSE;
          } */
...
// 	cmdheader->cfl = sizeof(FIS_REG_H2D)/sizeof(u32);	// Command FIS
// size 	cmdheader->w = 0;		// 0:device to host,read ;1:host
// to device,write 	cmdheader->c = 1;
//     cmdheader->p = 1;          //Software shall not set CH(pFreeSlot).P when
//     building queued ATA commands.
// 	cmdheader->prdbc = 0;
// 	cmdheader->prdtl =  1;	// PRDT entries count
...
// a new command 	while ((port->tfd & (ATA_DEV_BUSY | ATA_DEV_DRQ)) &&
// spin < 1000000)
// 	{
// 		spin++;
// 	}
...
int sata_read(int dev, u16 *buf, u64 sect, u32 count) {
  ...
  // cmdtbl->prdt_entry[0].dba = (u32)(buf);
  // cmdtbl->prdt_entry[0].dbc = 512-1;	// this value should always be set to 1
  // less than the actual value cmdtbl->prdt_entry[0].i = 0;
  int i = 0;
  /* 	for ( i=0; i<cmdheader->prdtl-1; i++)
          {
                  cmdtbl->prdt_entry[i].dba = (u32)(buf);
                  cmdtbl->prdt_entry[i].dbc = 8*1024-1;	// 8K bytes (this value
     should always be set to 1 less than the actual value)
                  cmdtbl->prdt_entry[i].i = 0 ;
                  buf += 8*1024;	// 8k bytes
                  count -= 16;	// 16 sectors
          } */
  ...
  /* 	while (sata_wait_flag)
          {
                  // In some longer duration reads, it may be helpful to spin on
     the DPS bit
                  // in the PxIS port field as well (1 << 5)
                  // lprintf("transfer byte count:");disp_int(cmdheader->prdbc);
                  // if (((port->ci & (1<<slot)) == 0)&&(cmdheader->prdbc >=
     512)){
                  // 	// fat("\nsuccess,transfer byte
     count:");disp_int(cmdheader->prdbc);
...
// 	cmdheader->cfl = sizeof(FIS_REG_H2D)/sizeof(u32);	// Command FIS
// size
// 	// cmdheader->w = (p->msg->type == DEV_READ) ? 0 : 1;		//
// 0:device to host,read ;1:host to device,write 	cmdheader->w = rw ;
// 	// disp_int(cmdheader->w);
// 	cmdheader->c = 0;
//     cmdheader->p = 1;          //Software shall not set CH(pFreeSlot).P when
//     building queued ATA commands.
...
// 	// cmdtbl->prdt_entry[0].dba = (u32)(buf);
// 	// cmdtbl->prdt_entry[0].dbc = 512-1;	// this value should always be
...
// 		cmdtbl->prdt_entry[i].dbc = 8*1024-1;	// 8K bytes (this value
// should always be set to 1 less than the actual value)
// cmdtbl->prdt_entry[i].i = 0 ; 		buf += 8*1024;	// 8k bytes
// count -= 16;	// 16 sectors
// 	}
// 	// Last entry202.117.249.6
// 	cmdtbl->prdt_entry[i].dba = (u32)(buf);
...
// 	// cmdfis->command = (p->msg->type == DEV_READ) ? ATA_CMD_READ_DMA_EX :
// ATA_CMD_WRITE_DMA_EX;
//  	cmdfis->command = (rw == 0) ? ATA_CMD_READ_DMA_EX :
//  ATA_CMD_WRITE_DMA_EX;
