...
static u32 get_next_clus_number(u32 current_clus) {
    ...
    readsect((void *)BUF_ADDR,
             bootPartStartSector + elf_fd.fat_start_sec + sec);
...
static void *read_cluster(void *dst, u32 current_clus) {
    ...
    int clus_start_sect =
        data_clus * bpb.BPB_SecPerClus + elf_fd.data_start_sec;
...
void fat32_init() {
    readsect((void *)&bpb,
             bootPartStartSector); // 读取FAT32的信息，写入BPB结构体
    // 当扇区字节大小不为512时判断为错误
    if (bpb.BPB_BytsPerSec != SECTSIZE) goto bad;
    elf_fd.fat_start_sec = bpb.BPB_RsvdSecCnt; // 计算fat表起始扇区号
    elf_fd.data_start_sec =
        elf_fd.fat_start_sec +
        bpb.BPB_FATSz32 * bpb.BPB_NumFATs; // 计算data起始扇区号
    elf_fd.first_clus = 0;
    return;
bad:
    while (1);
        ...
        for (struct Directory_Entry *p = (void *)BUF_ADDR; (void *)p < read_end;
            ...
            if (strncmp(p->DIR_Name, filename, strlen(filename)) ==
                ...
                file_clus = (u32)p->DIR_FstClusHI << 16 |
                            p->DIR_FstClusLO; // 记录kernel.bin所在的扇区号
                break;
            }
        }
...
int fat32_read(u32 offset, u32 lenth, void *buf) {
    ...
    u32 current_clus = fat32_find_clus_i(elf_fd.first_clus,
                                         clus_i); // 找到要读取的第一个簇的簇号
