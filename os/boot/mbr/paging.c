 ...
 * @LastEditTime: 2024-08-19 22:50:12
 * @FilePath: /minios/os/boot/mbr/paging.c
 * @Description:
 *
...
void paging(u32 MemChkBuf,u32 MCRNumber) {
	...
	u32 size = get_mem_info(MemChkBuf, MCRNumber);
	lprintf("memsize %d \n", size);

	// 在loader阶段，只使用前32M。超出50M+后会有bug，暂时没找到原因
...
u32 get_mem_size(u32 MemChkBuf, u32 MCRNumber) {
	u32 size = 0;
	struct ARDStruct *adr = (void *)MemChkBuf;  // 第一个结构体的起始指针指向MemChkBuf
	// 每次循环读取一个结构体的内容
	for (unsigned int cnt = 0; cnt < MCRNumber; adr++) {
		cnt++;
		print_mem(adr);
		// 当结构体的成员变量dwType为1的时候，代表为OS可用
		if (adr->dwType == 1)  {
			// 根据结构体的基地址的低32位以及长度(字节)的低32位计算内存大小
                        ...
                        u32 pte_flag)     // 权限
{
  // 分配PTE
	...
	if ((pde_ptr[PDX(laddr)] & PG_P) == 0) {  // 若虚拟地址计算出的页目录项没有东西，则分配
		...
		pde_ptr[PDX(laddr)] = pte_phy | PG_P | PG_USU | PG_RWW;  // 将分配的地址写回虚拟地址计算的页目录项
	}
	phyaddr_t pte_phy = PTE_ADDR(pde_ptr[PDX(laddr)]);  // 计算页目录项
	uintptr_t *pte_ptr = (uintptr_t *)pte_phy;  // 获取页目录项的内容，即页表的起始地址
	// PTE写入page的地址
	phyaddr_t page_phy;
