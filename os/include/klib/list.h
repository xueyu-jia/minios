...
PRIVATE inline void _list_insert(struct list_node* new, struct list_node* prev,
                                 struct list_node* next) {
  prev->next = new;
  next->prev = new;
  new->prev = prev;
...
PRIVATE inline void _list_remove(struct list_node* self, struct list_node* prev,
                                 struct list_node* next) {
  prev->next = next;
  next->prev = prev;
...
/// @brief
/// 返回entry在双向链表中的后一项，类型与entry相同，如果entry是最后一项(entry对应的node的后一项是head),返回NULL
...
/// @brief
/// 返回entry在双向链表中的前一项，类型与entry相同，如果entry是第一项(entry对应的node的前一项是head),返回NULL
/// @param head struct list_node* 链表头指针
/// @param entry Type* 数据项指针
/// @param member 链表节点在数据项Type中的成员名
...
/// @brief
/// 返回链表中第一个节点数据中key的值为value的有效数据指针，如果找不到，则返回NULL
/// @param head struct list_node* 链表头指针
/// @param type 数据项类型
