...
typedef struct s_proc {
    cpu_context
        context; // 因为x86版本使用偏移量访问变量，所以请将该变量放在开头
...
#define proc_kstacktop(proc) \
    ((STACK_FRAME*)(((char*)((proc) + 1) - P_STACKTOP)))
#define proc_stack_frame(proc) \
    ((STACK_FRAME*)(((char*)((proc) + 1) - P_STACKTOP)))
...
#define proc_real(proc)                                         \
    ({                                                          \
        ...
        PROCESS* proc_real =                                    \
            type == TYPE_THREAD ? &proc_table[ppid] : proc_src; \
        proc_real;                                              \
    })
