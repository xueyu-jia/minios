...
#define BITMAP_FULL 0xff              //位图的一块全被标记。分配对象时使用，跳过全部被标记的块         
...
/*
    ...
    返回值：
        ...
        1  要释放的地址已经空闲
*/
#define bitmap_is_free(bitmap, index) ((bitmap[index/BITMAP_ENTRY_BITS]&(1<<(index%BITMAP_ENTRY_BITS))) == 0)
...
/*
    ...
    返回值：
        位图的字节数目
*/
#define calc_bitmap_size(obj_count) ((obj_count / BITMAP_ENTRY_BITS + (obj_count%BITMAP_ENTRY_BITS!=0)) * sizeof(bitmap_entry_t))
