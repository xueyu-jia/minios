...
//  #define NR_SYS_CALL       31	//modified by mingxuan 2021-3-25
//  删除了4个系统调用接口：kmalloc、kmalloc_4k、malloc和free
// #define NR_SYS_CALL       41	//modified by xiaofeng 2021-9-8
// 增加了5个关于共享内存的接口   //modified by yingchi 2022.01.06 5 message
// queue API #define NR_SYS_CALL       42	//modified by xiaofeng 2022-1-10
// 增加了1个关于内核锁测试的接口
//  #define NR_SYS_CALL       44	//modified by xiaofeng 2022-1.18
//  增加了2个关于exec的接口
// #define NR_SYS_CALL       60	//modified by xiaofeng 2022-1.18
// 增加了2个关于exec的接口
// #define NR_SYS_CALL		  62	//modified by dongzhangqi 2023.5.17
// 增加两个关于pthread的接口
#define NR_SYS_CALL \
    64 // modified by sundong 2023.5.18 增加了初始化tty设备的系统调用
...
#define PageTblNumAddr \
    0x500 // 页表数量放在这个位置,必须与load.inc中一致					add by
          // visual 2016.5.11
// #define KernelPageTblAddr	0x200000 //内核页表物理地址，必须与load.inc中一致
...
#define ShareLinBase \
    (HeapLinLimitMAX + 0x100000) // 共享内存的起始地址	共享内存与堆顶有1M空闲
#define ShareLinLimitMAX (ShareLinBase + 0x100000) // 大小：1M
#define StackLinLimitMAX \
    (ShareLinLimitMAX    \
     + 0x100000) // 栈的大小： 1G-128M-4K（注意栈的基址和界限方向）
#define StackLinBase \
...
/***************目前线性地址布局*****************************		edit by visual
 *2016.5.25 进程代码		0 ~ 512M ,限制大小为512M 进程数据		512M ~
