...
static int orangefs_alloc_bitmap(int dev, int blk_base, int blk_nr, int cnt,
                                 int *pindex) {
...
PRIVATE int orange_alloc_imap_bit(struct super_block *sb) {
  ...
  int index;
  if (orangefs_alloc_bitmap(sb->sb_dev, 2, ORANGE_SB(sb)->nr_imap_blocks, 1,
                            &index) == 1) {
    return index;
  }
 ...
 *****************************************************************************/
PRIVATE int orange_alloc_smap_bit(struct super_block *sb,
                                  struct orange_inode_info *pin) {
  ...
  // return free_sect_nr;
  int index;
  pin->i_nr_blocks = orangefs_alloc_bitmap(
      sb->sb_dev, 2 + ORANGE_SB(sb)->nr_imap_blocks,
...
PRIVATE int orange_free_imap_bit(struct super_block *sb, int inode_nr) {
  ...
  // by xw, 18/12/27 char* fsbuf = kern_kmalloc(BLOCK_SIZE); // local array, to
  // substitute global fsbuf. added by xw, 18/12/27
 ...
 *****************************************************************************/
PRIVATE int orange_free_smap_bit(struct super_block *sb, int start_sect_nr,
                                 int nr_sects_to_free) {
  ...
  i = (((start_sect_nr - ORANGE_SB(sb)->n_1st_block + 1 - k) / 8) - j) /
      BLOCK_SIZE;
...
PUBLIC int orange_readdir(struct file_desc *file, unsigned int count,
                          struct dirent *start) {
...
PRIVATE void orange_fill_inode(struct inode *inode, struct super_block *sb,
                               int num) {
  inode->i_sb = sb;
  inode->i_dev = sb->sb_dev;
  inode->i_no = num;
  int blk_nr = 1 + 1 + ORANGE_SB(sb)->nr_imap_blocks +
               ORANGE_SB(sb)->nr_smap_blocks +
...
PRIVATE void orange_new_dir_entry(struct inode *dir_inode, int inode_nr,
  ...
  for (i = 0; i <= nr_dir_blks && i < dir_blk_total;
       i++)  // 此处取等,允许多读取一块
  {
    ...
    if (m > nr_dir_entries ||
        new_de) /* all entries have been iterated or free slot is found */
      break;
    brelse(bh);
  }
...
PUBLIC int orange_write(struct file_desc *file, unsigned int count,
                        const char *buf) {
  int pos = file->fd_pos;
  struct inode *pin = file->fd_dentry->d_inode;
  int sync_needed = 0;
