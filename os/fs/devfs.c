...
PUBLIC struct device *register_device(int dev, int type,
    ...
    list_for_each(&devices, dev_struct, dev_list) {
        ...
        for (int i = 0; minor_bit && i < 32; i++) {
            if (minor_bit & 1) {
                ...
                dirent_fill(dent, MAKE_DEV(dev_struct->dev_major, i),
                            strlen(dev_name));
...
PUBLIC void devfs_read_inode(struct inode *inode) {
    if (inode->i_no == 1) {
        ...
        int dev = inode->i_no;
        struct device *dev_struct = find_major_device(dev);
        if (dev_struct == NULL) { disp_str("error: cant find device"); }
        inode->i_mode = I_R | I_W;
        inode->i_b_cdev = dev;
        switch (dev_struct->dev_type) {
            case DEV_BLOCK_TYPE:
                inode->i_type = I_BLOCK_SPECIAL;
                inode->i_size = ((u64)hd_infos[MAJOR(dev) - DEV_HD_BASE]
                                     .part[MINOR(dev)]
                                     .size)
                                << SECTOR_SIZE_SHIFT;
                break;
