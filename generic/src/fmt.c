...
int strfmtcb(strfmt_handler_t *handler, char *buf, int size, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
...
int vstrfmtcb(strfmt_handler_t *handler, char *buf, int size, const char *fmt, va_list ap) {
    if (handler == NULL) { return -1; }
    int resp = stbsp_vsprintfcb(handler->callback, handler->user, buf, fmt, ap, size);
    return resp;
}
