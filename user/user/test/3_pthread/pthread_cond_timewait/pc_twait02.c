/*
 * 测试 pthread_cond_timewait 等待的线程被唤醒后能否正确的重新获取 mutex
 *
 * 1. 创建新线程
 ...
 * 5. 新线程 trylock mutex 应失败返回 -1。trylock 成功则说明
 * pthread_cond_timewait 未正确获取 mutex
...
void *thread_func(void *arg) {
  ...
  // 线程已运行
  started = 1;
  info(&logger, "thread[%d]: timewait for condition signal\n");
  ...
  // 检查被 pthread_cond_signal 唤醒后，pthread_cond_timewait 能否正确重新获取
  // mutex
  info(&logger, "thread[%d]: trylock mutex (should fail)\n");
  rval = pthread_mutex_trylock(&mutex);
  if (rval != -1) {
    error(&logger, "thread[%d]: should not be able to lock the mutex again\n",
          tid);
