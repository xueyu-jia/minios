...
void consumer() {  //该函数减少count数值
  printf("consumer:id=%d   mutex is locked by consumer         \n",
         pthread_self());
  pthread_mutex_lock(&mutex);
  //当条件不满足时等待
  printf("in consumer count=%d        \n", count);
  while (count <= 3) {  //防止虚假唤醒
    printf("consumer begin wait         \n");
    int t = 500;
    int sign_timeout =
        pthread_cond_timewait(&cond, &mutex, &t);  // time 1000 ticks = 1s
    printf("in consumer count=%d        \n", count);
...
void consumer2() {  //该函数减少count数值
  printf("consumer2:id=%d   mutex is locked by consumer2         \n",
         pthread_self());
  pthread_mutex_lock(&mutex);
  //当条件不满足时等待
  printf("in consumer2 count=%d        \n", count);
  while (count <= 6) {  //防止虚假唤醒
...
int main(int arg, char *argv[]) {
  ...
  sleep(
      50);  //由于线程内部的操作比较多，为了保证该线程先运行并且进入等待挂起，需要让主线程sleep足够的时间
