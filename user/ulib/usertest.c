...
int safe_write(const char *file, const int lineno, int fd, const void *buf,
    ...
    if (n < nbytes) {
        printf(
            "%s:%d: warning: write(%d, %p, %d) return value %d, expected %d\n",
...
int safe_read(const char *file, const int lineno, int fd, void *buf, int size) {
    ...
    if (rval < 0) {
        printf("%s:%s: read(%d, %p, %d) failed, return %d", file, lineno, fd,
               buf, size, rval);
        // if (cleanup_fn != NULL)
        // cleanup_fn();
        exit(-1);
...
int logger_init(logging *logger, const char *log_filename,
    ...
    if (fd < 0) {
        ...
        if (fd < 0) {
            printf(
                "logger_init: ERROR: failed to open or create %s, return %d\n",
                log_filename, fd);
            return -1;
        }
    }
