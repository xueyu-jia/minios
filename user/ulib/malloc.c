...
int malloc_initialized =
    0; // 用于malloc之前标志堆是否已经初始化，初始化了值为1，否则值为0.
// malloc_page ptable;
malloced mal;
free_vmem vmem;
...
u32 malloc(u32 size) // 为用户分配小内存
{
    ...
    if (vaddr == 0) udisp_str("malloc failed!\n");
...
void malloc_init() {
    u32 base = (u32)
        malloc_4k(); // 当malloc_4k系统调用传入参数为0时，返回内核态下堆底指针base;
...
static u32 vmalloc(u32 size) {
    ...
    for (i = 0; i < vmem.count;
         i++) // 先从空闲分区表中查找合适的内存块（空闲分区小于4K）
    {
    ...
    if (i >=
        vmem.count) // 没有找到合适的内存块，申请一个4K页面，并分配物理内存块，将剩余空闲块插入空闲表
    {
        u32 page_addr;
...
static void malloced_insert(
    u32 vaddr, u32 size) // 每分配一块，要将该块的信息记录在已经malloc的记录表中
{
    u32 i = mal.count;
...
static u32 vfree(u32 vaddr, u32 size) {
    ...
    if (i > 0) // 要释放的块的地址不是所有块中最低的，可能和前一块合并
    {
        ...
        if (vmem.vmem_table[i - 1].vaddr + vmem.vmem_table[i - 1].size ==
                vaddr &&
            ...
            if (vmem.vmem_table[i - 1].size >=
                num_4K) // 合并后的块大小等于4K，则释放该页面
            {
                free_page(i - 1);
            }
    ...
    if (i <
        vmem.count) //(i=0以及和前面的块不连续都走这个分支),
                    // 即和前面的块不连续或者前面无块，判断和后面的块是否连续
    {
        if (vaddr + size == vmem.vmem_table[i].vaddr &&
            ...
            if (vmem.vmem_table[i].size >=
                num_4K) // 当整个4K页面的分区都被释放时，释放相应的页面
            {
                free_page(i);
            }
    ...
    if (vmem.count <
        MAX_FREE) // 和前面后面的块都不连续且当前块数小于最大块数，直接插入块表
    {
...
static int free_page(u32 index) {
    ...
    if (vaddr + size ==
        hp.heap_lin_limit) // 当要释放的逻辑页面在堆顶时，释放相应的物理页
    {
        u32 tmp_vaddr;
...
static int free_ppage(
    u32 vaddr) ////当要释放的逻辑页面在堆顶时，释放相应的物理页
{
...
static u32
alloc_vpage() // 从空闲页面表中分配页面（这些页面已经映射到相应的物理页面）
{
    u32 i, vaddr;
...
static u32
alloc_hpage() // 从堆上分配虚拟页（堆指针limit+4k），同时要分配物理页面并进行映射
{
