...
struct address_space {
	...
	mem_pages pages; // 保存inode对应的cache page
};
...
struct vfs_inode{
	...
	struct address_space* i_mapping;	// inode cached pages, ordered by page offset
...
struct vfs_dentry{
	...
	char d_name[MAX_DNAME_LEN]; // 目录项文件/目录的名字
...
2. 另创建相应文件系统的源文件和头文件，调用缓冲区和VFS函数实现VFS相应的接口
