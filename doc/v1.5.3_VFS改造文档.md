...
1. int fd = open(const char* pathname, int flags) --> int fd = open(const char* pathname, int flags, ...)(第三个参数当flags含O_CREAT时需提供，类型为 int mode)
mode为创建文件的文件权限

2. int state = create(char* pathname) --> int fd = creat(const char* pathname)
语义改变：依照POSIX调整为创建并打开文件(如果已经存在则重写(原有内容清空)并打开)
