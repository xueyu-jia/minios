...
根据传入参数的不同,调用实例文件系统的create/mkdir,同时设置设备文件的信息
+ vfs_put_inode: 减少一个inode的引用计数,如果引用计数自减后>0,什么也不做, 否则:
1. 检查inode的i_nlink,如果为0则说明已经不再有inode引用此文件,只是当时还有进程使用,此时最后的使用引用计数(i_count)也已经释放,所以调用实例文件系统的delete_inode删除文件的磁盘空间
