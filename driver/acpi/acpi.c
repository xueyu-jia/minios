...
static void do_scan_acpi_sdst_visit_device(lai_nsnode_t *dev, lai_state_t *state) {
    lai_variable_t var = {};
            ...
            if (desc->large_item) {
                ...
                switch (desc->small_item_name) {
                    case 0x04: {
                        ...
                        char bitset[8] = {};
                        for (int i = 0; i < 8; ++i) { bitset[i] = !!(d->irq_mask & BIT(i)) + '0'; }
                        acpi_put_dev_prop("IRQ Mask: 0b%08.8s", bitset);
                        acpi_put_dev_prop("Wake Capability: %s",
...
void scan_acpi_sdst_impl(lai_nsnode_t *node, lai_state_t *state) {
    if (lai_ns_get_node_type(node) == LAI_NODETYPE_DEVICE) {
        do_scan_acpi_sdst_visit_device(node, state);
    }

    struct lai_ns_child_iterator iter = {};
    lai_initialize_ns_child_iterator(&iter, node);
    lai_nsnode_t *ch_node = NULL;
    while ((ch_node = lai_ns_child_iterate(&iter))) { scan_acpi_sdst_impl(ch_node, state); }
}

void scan_acpi_sdst() {
    lai_state_t state = {};
    lai_init_state(&state);
    scan_acpi_sdst_impl(lai_ns_get_root(), &state);
