...
#include <driver/acpi/lai/drivers/timer.h>
...
lai_api_error_t lai_start_pm_timer() {
    ...
    if (fadt->pm_timer_length != 4) return LAI_ERROR_UNSUPPORTED;
    ...
    if (fadt->flags & (1 << 8)) extended_timer = 1;
...
lai_api_error_t lai_stop_pm_timer() {
    if (!supported) return LAI_ERROR_UNSUPPORTED;
