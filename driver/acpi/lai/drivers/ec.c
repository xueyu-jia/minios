...
#include <driver/acpi/lai/drivers/ec.h>
...
static void poll_ibf(struct lai_ec_driver *driver) {
    while (1) {
        ...
        if ((status & ACPI_EC_STATUS_IBF) == 0) return;
    }
}
...
static void poll_obf(struct lai_ec_driver *driver) {
    while (1) {
        uint8_t status = laihost_inb(driver->cmd_port);
        if ((status & ACPI_EC_STATUS_OBF) != 0) return;
    }
}
...
static void enable_burst(struct lai_ec_driver *driver) {
    ...
    poll_obf(driver);
    if (laihost_inb(driver->data_port) != 0x90) lai_panic("Enabling EC Burst Mode Failed");
...
static uint16_t readw(uint64_t offset, void *userptr) {
    enable_burst(userptr);
    uint16_t ret =
        (uint16_t)lai_read_ec(offset, userptr) | ((uint16_t)lai_read_ec(offset + 1, userptr) << 8);
    disable_burst(userptr);
    return ret;
}
...
static uint64_t readq(uint64_t offset, void *userptr) {
    enable_burst(userptr);
    uint64_t ret = lai_read_ec(offset, userptr) |
                   ((uint64_t)lai_read_ec(offset + 1, userptr) << 8) |
