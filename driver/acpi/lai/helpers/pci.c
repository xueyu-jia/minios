...
#include <driver/acpi/lai/helpers/pci.h>
#include <driver/acpi/lai/helpers/resource.h>
...
int lai_pci_route(acpi_resource_t *dest, uint16_t seg, uint8_t bus, uint8_t slot,
    ...
    if (!pin || pin > 4) return 1;

    if (lai_pci_route_pin(dest, seg, bus, slot, function, pin)) return 1;
    return 0;
}
...
lai_api_error_t lai_pci_route_pin(acpi_resource_t *dest, uint16_t seg, uint8_t bus, uint8_t slot,
    ...
    if (!handle) return LAI_ERROR_NO_SUCH_NODE;
...
lai_api_error_t lai_pci_parse_prt(struct lai_prt_iterator *iter) {
    ...
    if (lai_obj_get_pkg(iter->prt, iter->i, &prt_entry)) return LAI_ERROR_UNEXPECTED_RESULT;

    iter->i++;

    if (lai_obj_get_pkg(&prt_entry, 0, &prt_entry_addr)) return LAI_ERROR_UNEXPECTED_RESULT;
    if (lai_obj_get_pkg(&prt_entry, 1, &prt_entry_pin)) return LAI_ERROR_UNEXPECTED_RESULT;
    if (lai_obj_get_pkg(&prt_entry, 2, &prt_entry_type)) return LAI_ERROR_UNEXPECTED_RESULT;
    if (lai_obj_get_pkg(&prt_entry, 3, &prt_entry_index)) return LAI_ERROR_UNEXPECTED_RESULT;

    uint64_t addr;
    if (lai_obj_get_integer(&prt_entry_addr, &addr)) return LAI_ERROR_UNEXPECTED_RESULT;

    iter->slot = (addr >> 16) & 0xFFFF;
    iter->function = addr & 0xFFFF;

    if (iter->function == 0xFFFF) iter->function = -1;

    uint64_t pin;
    if (lai_obj_get_integer(&prt_entry_pin, &pin)) return LAI_ERROR_UNEXPECTED_RESULT;
    ...
    if (type == LAI_TYPE_INTEGER) { // direct routing to GSI
        uint64_t gsi;
        if (lai_obj_get_integer(&prt_entry_index, &gsi)) return LAI_ERROR_UNEXPECTED_RESULT;
        ...
        uint64_t res_index;
        if (lai_obj_get_handle(&prt_entry_type, &link_handle)) return LAI_ERROR_UNEXPECTED_RESULT;
        if (lai_obj_get_integer(&prt_entry_index, &res_index)) return LAI_ERROR_UNEXPECTED_RESULT;
        ...
        while (!lai_resource_iterate(&view)) {
            if (current == res_index) {
                ...
                if (type != LAI_RESOURCE_IRQ) return LAI_ERROR_UNEXPECTED_RESULT;
                if (lai_resource_next_irq(&view)) return LAI_ERROR_UNEXPECTED_RESULT;
...
lai_nsnode_t *lai_pci_find_device(lai_nsnode_t *bus, uint8_t slot, uint8_t function,
    ...
    while ((node = lai_ns_child_iterate(&iter))) {
        ...
        if (adr_handle) {
            ...
            if (adr_result == device_adr) return node;
        }
    }
...
lai_nsnode_t *lai_pci_find_bus(uint16_t seg, uint8_t bus, lai_state_t *state) {
    ...
    while ((node = lai_ns_child_iterate(&iter))) {
        ...
        if (seg_result == seg && bbn_result == bus) { return node; }
    }
