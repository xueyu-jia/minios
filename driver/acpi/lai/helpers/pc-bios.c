...
#include <driver/acpi/acpispec/tables.h>
#include <driver/acpi/lai/core.h>
#include <driver/acpi/lai/helpers/pc-bios.h>

static uint8_t lai_bios_calc_checksum(void *ptr, size_t size) {
    uint8_t sum = 0;
    for (size_t i = 0; i < size; i++) sum += ((uint8_t *)ptr)[i];
    return sum;
}
...
lai_api_error_t lai_bios_detect_rsdp_within(uintptr_t base, size_t length,
    ...
    for (size_t off = 0; off < length; off += 16) {
        ...
        if (memcmp(rsdp->signature, "RSD PTR ", 8)) continue;

        if (lai_bios_calc_checksum(rsdp, sizeof(acpi_rsdp_t))) continue;
        ...
        if (!rsdp->revision) {
            ...
            if (lai_bios_calc_checksum(xsdp, sizeof(acpi_xsdp_t))) continue;
...
lai_api_error_t lai_bios_detect_rsdp(struct lai_rsdp_info *info) {
    ...
    if (!(e = lai_bios_detect_rsdp_within(ebda_base, 0x400, info))) return LAI_ERROR_NONE;
    LAI_ENSURE(e == LAI_ERROR_END_REACHED);

    if (!(e = lai_bios_detect_rsdp_within(0xE0000, 0x20000, info))) return LAI_ERROR_NONE;
    LAI_ENSURE(e == LAI_ERROR_END_REACHED);
