...
#include <driver/acpi/lai/helpers/sci.h>
...
uint16_t lai_get_sci_event(void) {
    ...
    if (!laihost_inw || !laihost_outw) lai_panic("lai_read_event() requires port I/O");
...
void lai_set_sci_event(uint16_t value) {
    ...
    if (!laihost_inw || !laihost_outw) lai_panic("lai_set_event() requires port I/O");
    ...
    if (instance->fadt->pm1a_event_block) laihost_outw(a, value);

    if (instance->fadt->pm1b_event_block) laihost_outw(b, value);
...
int lai_enable_acpi(uint32_t mode) {
    ...
    if (handle) {
        ...
        if (!lai_eval(NULL, handle, &state)) lai_debug("evaluated \\_SB_._INI");
    ...
    if (!lai_current_instance()->is_hw_reduced) {
        if (!laihost_inw || !laihost_outb) lai_panic("lai_enable_acpi() requires port I/O");
        ...
        for (size_t i = 0; i < 100; i++) {
            if (laihost_inw(instance->fadt->pm1a_control_block) & ACPI_ENABLED) break;
...
int lai_disable_acpi(void) {
    ...
    if (!laihost_inw || !laihost_outw) lai_panic("lai_disable_acpi() requires port I/O");
...
int lai_evaluate_sta(lai_nsnode_t *node) {
    ...
    if (handle) {
        ...
        if (lai_obj_get_integer(&result, &sta)) lai_panic("_STA returned non-integer object");
    }
...
void lai_init_children(lai_nsnode_t *parent) {
    ...
    while ((node = lai_ns_child_iterate(&iter))) {
        if (lai_ns_get_node_type(node) == LAI_NODETYPE_DEVICE) {
            ...
            if (sta & ACPI_STA_PRESENT || sta & ACPI_STA_FUNCTION) lai_init_children(node);
        }
    }
}
