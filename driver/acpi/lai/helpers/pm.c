...
#include <driver/acpi/lai/helpers/pm.h>
...
lai_api_error_t lai_enter_sleep(uint8_t sleep_state) {
    ...
    if (instance->is_hw_reduced) {
        ...
        if (!control_reg->base) return LAI_ERROR_UNSUPPORTED;
        ...
        switch (control_reg->address_space) {
            case ACPI_GAS_IO:
                if (!laihost_outb) lai_panic("lai_enter_sleep() requires port I/O");

                laihost_outb(control_reg->base, value);
                break;
            case ACPI_GAS_MMIO:
                if (!laihost_map) lai_panic("lai_enter_sleep() requires laihost_map()");
        ...
        if (!laihost_inw || !laihost_outw) lai_panic("lai_enter_sleep() requires port I/O");
...
lai_api_error_t lai_acpi_reset() {
    ...
    if (!fadt) {
        if (!laihost_scan) lai_panic("laihost_scan is required for lai_acpi_reset");

        fadt = laihost_scan("FACP", 0); // If fadt hasn't been initialized yet get it, AML doesn't
                                        // have to be scanned for reset
        if (!fadt) lai_panic("Buggy BIOS does not provide FADT");
    }
    ...
    switch (fadt->reset_register.address_space) {
        case ACPI_GAS_MMIO: {
            if (!laihost_map) lai_panic("laihost_map is required for lai_acpi_reset");
        ...
        case ACPI_GAS_IO:
            if (!laihost_outb) lai_panic("laihost_outb is required for lai_acpi_reset");
