...
static unsigned int lai_hash_string(const char *str, size_t n) {
    ...
    unsigned int x = 5381;
    for (size_t i = 0; i < n; i++) x = ((x << 5) + x) + str[i];
    return x;
}
...
lai_nsnode_t *lai_create_nsnode_or_die(void) {
    ...
    if (!node) lai_panic("could not allocate new namespace node");
    return node;
}
...
lai_api_error_t lai_install_nsnode(lai_nsnode_t *node) {
    ...
    if (instance->ns_size == instance->ns_capacity) {
        ...
        if (!new_array) lai_panic("could not reallocate namespace table");
...
void lai_uninstall_nsnode(lai_nsnode_t *node) {
    ...
    for (size_t i = 0; i < instance->ns_size; i++) {
        if (instance->ns_array[i] == node) instance->ns_array[i] = NULL;
    }
    ...
    if (parent) {
        ...
        for (;;) {
            if (lai_hashtable_chain_advance(&parent->children, h, &chain))
                lai_panic(
                    "child node is missing from parent's hash table"
        ...
        while (!lai_hashtable_chain_advance(&parent->children, h, &chain)) {
            ...
            if (child == node)
                lai_panic(
                    "child node appears multiple times in parent's hash table"
                    " during lai_uninstall_nsnode()");
        }
    }
}
...
lai_nsnode_t *lai_ns_get_child(lai_nsnode_t *parent, const char *name) {
    ...
    while (!lai_hashtable_chain_advance(&parent->children, h, &chain)) {
        lai_nsnode_t *child = lai_hashtable_chain_get(&parent->children, h, &chain);
        if (!memcmp(child->name, name, 4)) return child;
    }
    return NULL;
}
...
void lai_amlname_iterate(struct lai_amlname *amln, char *out) {
    ...
    for (int i = 0; i < 4; i++) out[i] = amln->it[i];
    amln->it += 4;
}
...
char *lai_stringify_amlname(const struct lai_amlname *in_amln) {
    ...
    if (!str) lai_panic("could not allocate in lai_stringify_amlname()");

    int n = 0;
    if (amln.is_absolute) str[n++] = '\\';
    for (int i = 0; i < amln.height; i++) str[n++] = '^';

    if (!lai_amlname_done(&amln)) {
        for (;;) {
            lai_amlname_iterate(&amln, &str[n]);
            n += 4;
            if (lai_amlname_done(&amln)) break;
            str[n++] = '.';
        }
    }
    str[n++] = '\0';
    ...
    if (n != (int)max_length) str = laihost_realloc(str, n, max_length);
...
char *lai_stringify_node_path(lai_nsnode_t *node) {
    ...
    if (!node->parent) {
        ...
        if (!str) lai_panic("could not allocate in lai_stringify_node_path()");
        lai_strcpy(str, "\\");
        return str;
    }
    ...
    size_t num_segs = 0;
    for (current = node; current->parent; current = current->parent) num_segs++;
...
lai_nsnode_t *lai_do_resolve(lai_nsnode_t *ctx_handle, const struct lai_amlname *in_amln) {
    ...
    if (amln.search_scopes) {
        ...
        if (debug_resolution) lai_debug("resolving %s by searching through scopes", segment);
        ...
        if (lai_amlname_done(&amln)) return current;
    ...
    for (;;) {
        ...
        if (lai_amlname_done(&amln)) {
            ...
            if (parent->type == LAI_NAMESPACE_ALIAS) {
                lai_warn(
                    "resolution of new object name traverses Alias(),"
...
void lai_create_namespace(void) {
    if (!laihost_scan) lai_panic("lai_create_namespace() needs table management functions");

    struct lai_instance *instance = lai_current_instance();

    // we need the FADT
    instance->fadt = laihost_scan("FACP", 0);
    if (!instance->fadt) { lai_panic("unable to find ACPI FADT."); }
    ...
    if (!dsdt_table) lai_panic("unable to find ACPI DSDT.");
...
static struct lai_aml_segment *lai_load_table(void *ptr, int index) {
    ...
    if (!amls) lai_panic("could not allocate memory for struct lai_aml_segment");
...
lai_nsnode_t *lai_resolve_path(lai_nsnode_t *ctx_handle, const char *path) {
    lai_nsnode_t *current = ctx_handle;
    if (!current) current = lai_current_instance()->root_node;
    ...
    for (;;) {
        ...
        int k;
        for (k = 0; k < 4; k++) {
            if (!lai_is_name(*path)) break;
            segment[k] = *(path++);
        }

        // ACPI pads names with trailing underscores.
        while (k < 4) segment[k++] = '_';
        segment[4] = '\0';
...
int lai_check_device_pnp_id(lai_nsnode_t *dev, lai_variable_t *pnp_id, lai_state_t *state) {
    ...
    if (id.type == LAI_INTEGER && pnp_id->type == LAI_INTEGER) {
        if (id.integer == pnp_id->integer) { ret = 0; }
    } else if (id.type == LAI_STRING && pnp_id->type == LAI_STRING) {
        if (!lai_strcmp(lai_exec_string_access(&id), lai_exec_string_access(pnp_id))) { ret = 0; }
    }
