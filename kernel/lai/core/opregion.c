...
static size_t lai_calculate_access_width(lai_nsnode_t *field) {
    ...
    switch (field->fld_flags & 0xF) {
        ...
        case FIELD_ANY_ACCESS: {
            ...
            access_size = 1;
            if (field->fld_size > 1) access_size = 1 << (32 - __builtin_clz(field->fld_size - 1));

            size_t max_access_width = 32;
            if (opregion->op_address_space == ACPI_OPREGION_MEMORY) max_access_width = 64;

            if (access_size > max_access_width) access_size = max_access_width;

            if (access_size < 8) access_size = 8;
...
static void lai_get_pci_params(lai_nsnode_t *opregion, uint64_t *seg, uint64_t *bbn,
    ...
    if (!device) lai_panic("lai_get_pci_params: Couldn't get device");

    lai_nsnode_t *bus = lai_ns_get_parent(device);
    if (!bus) lai_panic("lai_get_pci_params: Couldn't get bus");

    lai_nsnode_t *root_bus = lai_find_parent_root_of(bus, &state);
    if (!root_bus) lai_panic("lai_get_pci_params: Couldn't get root bus");
    ...
    if (adr_handle) {
        ...
        if (adr) *adr = address_number.integer;
    }
}
...
static uint64_t lai_perform_read(lai_nsnode_t *opregion, size_t access_size, size_t offset) {
    ...
    if (opregion->op_override) {
        if (instance->trace & LAI_TRACE_IO)
            lai_debug(
                "lai_perform_read: %lu-bit read from overridden opregion at %lx (address "
                "space %02u)",
                access_size, opregion->op_base + offset, opregion->op_address_space);
        switch (access_size) {
            case 8:
                value =
                    opregion->op_override->readb(opregion->op_base + offset, opregion->op_userptr);
                break;
            case 16:
                value =
                    opregion->op_override->readw(opregion->op_base + offset, opregion->op_userptr);
                break;
            case 32:
                value =
                    opregion->op_override->readd(opregion->op_base + offset, opregion->op_userptr);
                break;
            case 64:
                value =
                    opregion->op_override->readq(opregion->op_base + offset, opregion->op_userptr);
                break;
            default:
                lai_panic("invalid access size");
        }
        ...
        switch (opregion->op_address_space) {
            ...
            case ACPI_OPREGION_IO: {
                ...
                if (!laihost_inb || !laihost_inw || !laihost_ind)
                    lai_panic(
                        "lai_perform_read: The laihost_in{b,w,d} functions need to be "
            ...
            case ACPI_OPREGION_PCI: {
                ...
                if (instance->trace & LAI_TRACE_IO)
                    lai_debug(
                        "lai_perform_read: %lu-bit read from PCI config of "
                ...
                if (!laihost_pci_readb || !laihost_pci_readw || !laihost_pci_readd)
                    lai_panic(
                        "lai_perform_read: The laihost_pci_read{b,w,d} functions need to be "
...
static void lai_perform_write(lai_nsnode_t *opregion, size_t access_size, size_t offset,
    ...
    if (opregion->op_override) {
        if (instance->trace & LAI_TRACE_IO)
            lai_debug(
                "lai_perform_write: %lu-bit write of %lx to overridden opregion at %lx "
                "(address space %02u)",
        ...
        switch (opregion->op_address_space) {
            ...
            case ACPI_OPREGION_IO: {
                ...
                if (!laihost_outb || !laihost_inw || !laihost_outd)
                    lai_panic(
                        "lai_perform_write: The laihost_out{b,w,d} functions need to be "
...
void lai_write_field_internal(uint8_t *source, lai_nsnode_t *field) {
    ...
    while (progress < field->fld_size) {
        uint64_t bit_offset = (field->fld_offset + progress) & (access_size - 1);
        size_t access_bits = LAI_MIN(field->fld_size - progress, access_size - bit_offset);
        size_t mask = ((1ull << access_bits) - 1) << bit_offset;
...
void lai_read_field(lai_variable_t *destination, lai_nsnode_t *field) {
    ...
    if (bytes > 8) {
        ...
        uint64_t value = 0;
        for (size_t i = 0; i < bytes; i++) { value |= (uint64_t)buf[i] << (i * 8); }
...
void lai_write_field(lai_nsnode_t *field, lai_variable_t *source) {
    if (source->type == LAI_BUFFER) {
        ...
        for (size_t i = 0; i < 8; i++) { buf[i] = (source->integer >> (i * 8)) & 0xFF; }
