...
void lai_finalize_state(lai_state_t *state) {
    while (state->ctxstack_ptr >= 0) lai_exec_pop_ctxstack_back(state);
    while (state->blkstack_ptr >= 0) lai_exec_pop_blkstack_back(state);
    while (state->stack_ptr >= 0) lai_exec_pop_stack_back(state);
...
static lai_api_error_t lai_exec_reduce_op(int opcode, lai_state_t *state,
    ...
    switch (opcode) {
        ...
        case LEQUAL_OP: {
            ...
            if (err != LAI_ERROR_NONE) return LAI_ERROR_ILLEGAL_ARGUMENTS;
        ...
        case INDEX_OP: {
            ...
            switch (object.type) {
                ...
                case LAI_BUFFER:
                    ...
                    break;
                case LAI_PACKAGE:
                    if (n >= lai_exec_pkg_size(&object)) lai_panic("package Index() out of bounds");
                ...
                default:
                    lai_panic(
                        "Index() is only defined for buffers, strings and packages"
        ...
        case MATCH_OP: {
            ...
            if (package.type != LAI_PACKAGE) return LAI_ERROR_UNEXPECTED_RESULT;
            ...
            for (size_t i = start_index; i < package_size; i++) {
                ...
                if (res != LAI_ERROR_NONE) return LAI_ERROR_ILLEGAL_ARGUMENTS;
        ...
        case CONCATRES_OP: {
            ...
            if (buf1_size == 1 || buf2_size == 1) return LAI_ERROR_UNEXPECTED_RESULT;

            if (buf1_size == 0)
                buf1_size =
                    2; // Make it 2 so memcpy will actually copy 0 zero bytes since it is empty

            if (buf2_size == 0) buf2_size = 2;
            ...
            uint8_t check = 0;
            for (size_t i = 0; i < (result_size - 1); i++) check += result_buffer[i];
        ...
        case NOTIFY_OP: {
            ...
            if (laihost_handle_global_notify) laihost_handle_global_notify(node, code.integer);
        ...
        case (EXTOP_PREFIX << 8) | STALL_OP: {
            ...
            if (!time.integer) time.integer = 1;
        ...
        case (EXTOP_PREFIX << 8) | FATAL_OP: {
            ...
            if (!fatal_type.integer) fatal_type.integer = 0;

            if (!fatal_data.integer) fatal_data.integer = 0;

            if (!fatal_arg.integer) fatal_arg.integer = 0;
...
static lai_api_error_t lai_exec_run(lai_state_t *state) {
    while (lai_exec_peek_stack_back(state)) {
        ...
        lai_api_error_t e;
        if ((e = lai_exec_process(state))) return e;
    }
...
static size_t lai_parse_varint(size_t *out, uint8_t *code, int *pc, int limit) {
    ...
    if (!sz) {
        *out = (size_t)(code[*pc] & 0x3F);
        ...
        if (*pc + 2 > limit) return 1;
        *out = (size_t)(code[*pc] & 0x0F) | (size_t)(code[*pc + 1] << 4);
        *pc += 2;
        return 0;
    } else if (sz == 2) {
        if (*pc + 3 > limit) return 1;
        *out = (size_t)(code[*pc] & 0x0F) | (size_t)(code[*pc + 1] << 4) |
               (size_t)(code[*pc + 2] << 12);
        *pc += 3;
        return 0;
...
static lai_api_error_t lai_exec_process(lai_state_t *state) {
    ...
    if (block->pc > block->limit)
        lai_panic(
            "execution escaped out of code range"
            " [0x%lx, limit 0x%lx])",
    ...
    if (item->kind == LAI_POPULATE_STACKITEM) {
        ...
        if (block->pc == block->limit) {
            if (lai_exec_reserve_opstack(state)) return LAI_ERROR_OUT_OF_MEMORY;
        ...
        if (k == 1) {
            ...
            if (initial_size < 0) lai_panic("buffer initializer has negative size");
        ...
        if (block->pc == block->limit) {
            if (!item->pkg_want_result) lai_exec_pop_opstack_back(state);
        ...
        if (!item->op_arg_modes[k]) {
            ...
            if (error != LAI_ERROR_NONE) { return error; }
            lai_exec_pop_opstack(state, k);
        ...
        if (k == 1) {
            ...
            while (1) {
                ...
                if (!method_item) lai_panic("Return() outside of control method()");
                if (method_item->kind == LAI_METHOD_STACKITEM) break;
...
static lai_api_error_t lai_exec_parse(int parse_mode, lai_state_t *state) {
    ...
    if (parse_mode == LAI_IMMEDIATE_BYTE_MODE) {
        uint8_t value = 0;
        if (lai_parse_u8(&value, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
        ...
        uint16_t value = 0;
        if (lai_parse_u16(&value, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
        ...
        uint32_t value = 0;
        if (lai_parse_u32(&value, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
    ...
    if (lai_is_name(method[pc])) {
        struct lai_amlname amln;
        if (lai_parse_name(&amln, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
        ...
        if (instance->trace & LAI_TRACE_OP) path = lai_stringify_amlname(&amln);
    ...
    if (method[pc] == EXTOP_PREFIX) {
        if (pc + 1 == block->limit) lai_panic("two-byte opcode on method boundary");
    ...
    switch (opcode) {
        ...
        case QWORDPREFIX: {
            ...
            switch (opcode) {
                case BYTEPREFIX: {
                    uint8_t temp;
                    if (lai_parse_u8(&temp, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
                    value = temp;
                    break;
                }
                case WORDPREFIX: {
        ...
        case STRINGPREFIX: {
            ...
            while (pc + n < (size_t)block->limit && method[pc + n]) n++;
            if (pc + n == (size_t)block->limit) lai_panic("unterminated string in AML code");
            data_pc = pc;
            pc += n + 1;
        ...
        case RETURN_OP: {
            if (lai_exec_reserve_stack(state)) return LAI_ERROR_OUT_OF_MEMORY;
            lai_exec_commit_pc(state, pc);
        ...
        case CONTINUE_OP: {
            ...
            while (1) {
                ...
                if (!loop_item) lai_panic("Continue() outside of While()");
                if (loop_item->kind == LAI_LOOP_STACKITEM) break;
        ...
        case BREAK_OP: {
            ...
            while (1) {
                loop_item = lai_exec_peek_stack(state, m);
                if (!loop_item) lai_panic("Break() outside of While()");
                if (loop_item->kind == LAI_LOOP_STACKITEM) break;
                if (loop_item->kind != LAI_COND_STACKITEM && loop_item->kind != LAI_LOOP_STACKITEM)
        ...
        case IF_OP: {
            ...
            size_t if_size = 0;
            size_t else_size = 0;
            if (lai_parse_varint(&if_size, method, &pc, limit)) return LAI_ERROR_EXECUTION_FAILURE;
            if_pc = pc;
        ...
        case (EXTOP_PREFIX << 8) | DEVICE: {
            ...
            if (invocation) lai_list_link(&invocation->per_method_list, &node->per_method_item);
...
lai_api_error_t lai_eval_args(lai_variable_t *result, lai_nsnode_t *handle, lai_state_t *state,
    ...
    switch (handle->type) {
        case LAI_NAMESPACE_NAME:
            ...
            if (result) lai_obj_clone(result, &handle->object);
            return LAI_ERROR_NONE;
        case LAI_NAMESPACE_METHOD: {
            ...
            if (e == LAI_ERROR_NONE && result) lai_var_move(result, &method_result);
            return e;
        }
