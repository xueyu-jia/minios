...
static int page_filemap(memory_page_t* target, struct address_space* file_mapping, int create) {
    buf_head* bh = NULL;
    ...
    if ((PAGE_SIZE % size) || nr > MAX_BUF_PAGE) {
        kprintf("block size must be 512/1024/2048/4096");
    }
...
// req. inode lock; file page cache list lock
int generic_file_readpage(struct address_space* file_mapping, memory_page_t* target) {
...
// req. inode lock; file page cache list lock
int generic_file_writepage(struct address_space* file_mapping, memory_page_t* target) {
