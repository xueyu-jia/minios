...
int kern_sigsend(int pid, Sigaction* action) {
    ...
    if (task->sig_handler[action->sig] == SIG_IGN || (task->sig_set & sig_bit)) { return -1; }
...
void process_signal() {
    ...
    frame_ptr -= sizeof(stack_frame_t);
    memcpy((void*)frame_ptr, sigproc_user_regs, sizeof(stack_frame_t));
