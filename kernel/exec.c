...
static int exec_count_args(char* const* pstr, u32* count);
static int exec_copy_args(char* const* pstr, char** parr, char* dst, u32 offset);
...
static int exec_load(u32 fd, const elf32_hdr_t* eh, const elf32_phdr_t* ph) {
    ...
    for (ph_num = 0, phdr = ph; ph_num < eh->e_phnum; ph_num++, phdr++) {
        if (phdr->p_type == PT_LOAD) {
            ...
            if (file_page_size) {
                addr = do_mmap(mem_start, file_page_size, prot, MAP_PRIVATE | MAP_FIXED, fd,
