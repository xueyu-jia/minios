...
#define ASSERT_PINFO_IMPL(method, fmt, ...) \
    MH_EXPAND(MH_CONCAT(_, method)(__FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__))
...
#define UNIMPLEMENTED_IMPL(N, ...) MH_EXPAND(MH_CONCAT(UNIMPLEMENTED_IMPL, N)(__VA_ARGS__))
