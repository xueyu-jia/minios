...
/*__attribute__((noreturn, format(printf, 1, 2))) */ NORETURN void lai_panic(const char *, ...);
...
#define LAI_TRY(expr)                                    \
    ({                                                   \
        ...
        if (try_res_ != LAI_ERROR_NONE) return try_res_; \
        try_res_;                                        \
    })
...
static inline void lai_cleanup_free_string(char **v) {
    if (*v) laihost_free(*v, lai_strlen(*v) + 1);
}
