...
*/
// 判断c是否是非法字符
static inline int fat_bad_char(char c) {
    return ((c < 0x20) || (strchr("\"*/\\:<>?|", c) != NULL));
}

static inline int fat_skip_char(char c) {
    return ((c == ' ') || (c == '.'));
}

// 判断c是否需要在短文件名替换为"_"
static inline int fat_replace_char(char c) {
...
static void fat_update_datetime(u32 timestamp, u16* date, u16* time, u8* centi_sec) {
    struct tm time_s;
    gmtime(timestamp, &time_s);
    if (date != NULL) {
        *date =
            ((time_s.tm_year + 1900 - 1980) << 9) | ((time_s.tm_mon + 1) << 5) | (time_s.tm_mday);
    }
    if (time != NULL) {
        *time = ((time_s.tm_hour) << 11) | ((time_s.tm_min) << 5) | (time_s.tm_sec >> 1);
    }
    if (centi_sec != NULL) { *centi_sec = (time_s.tm_sec & 1) * 100; }
}

static u32 fat_read_datetime(u16 date, u16 time, u8 centi_sec) {
    struct tm time_s;
...
static inline struct fat_info* alloc_fat_info(int cluster_start) {
...
static char* fat_bread(struct super_block* sb, int block, buf_head** bh) {
    buf_head* b = NULL;
    u32 dev = sb->sb_dev;
...
// value == -1 for read
static int read_write_fat(struct super_block* sb, int cluster, int value) {
    int dev = sb->sb_dev, next;
    UNUSED(dev);
    if (cluster > FAT_SB(sb)->max_cluster) { return 0; }
...
static int add_new_cluster(struct super_block* sb, int tail) {
    ...
    struct fat_fsinfo* info_buf =
        (struct fat_fsinfo*)(fat_bread(sb, FAT_SB(sb)->fsinfo_block, &bh) + FS_INFO_SECTOR_OFFSET);
...
static int fill_fat_info(struct super_block* sb, int cluster_start, struct fat_info** info) {
...
static int fat_get_cluster(struct inode* inode, int cluster, int new_space) {
...
int fat32_get_block(struct inode* inode, u32 iblock, int create) {
...
static int fat_entry_offset_by_ino(struct inode* dir, u32 ino) {
...
static struct fat_dir_entry* fat_get_entry(struct inode* dir, int* start, buf_head** bh,
                                           char* full_name) {
    ...
    for (; (u64)(*start) * FAT_ENTRY_SIZE < dir->i_size; (*start)++) {
        ...
        if (ds->attr == ATTR_LNAME) {
            ...
            offset += 2;
            if (ds->order & DIR_LDIR_END) { uni_name[offset] = 0; }
            n_slot--;
...
static int fat_find_free(struct inode* dir, int num) {
    buf_head* bh = NULL;
    struct fat_dir_slot* ds;
    ...
    for (start = 0;; ++start) {
        ...
        if (end_flag || (ds->order == DIR_DELETE) || (ds->order == 0) ||
            ...
            if (end_flag == 0 && ((ds->order == 0) || ((u64)start * FAT_ENTRY_SIZE >= dir->i_size)))
                end_flag = 1;
...
static int fat_gen_shortname(struct inode* dir, const char* fullname, char* shortname) {
...
static struct inode* fat_add_entry(struct inode* dir, const char* name, int is_dir, u32 timestamp) {
...
void fat32_read_inode(struct inode* inode) {
    struct super_block* sb = inode->i_sb;
    buf_head* bh = NULL;
    ...
    if (!inode->i_size) { inode->i_size = cnt * FAT_SB(sb)->cluster_block * sb->sb_blocksize; }
    inode->i_nlink = 1;
...
int fat32_sync_inode(struct inode* inode) {
    if (inode->i_no == FAT_ROOT_INO) { return 0; }
    struct super_block* sb = inode->i_sb;
    int entry_block = sb->sb_blocksize / FAT_ENTRY_SIZE;
    buf_head* bh = NULL;
    u32 ino = inode->i_no;
...
void fat32_put_inode(struct inode* inode) {
...
void fat32_delete_inode(struct inode* inode) {
...
struct dentry* fat32_lookup(struct inode* dir, const char* filename) {
    // 1. format
...
int fat32_create(struct inode* dir, struct dentry* dentry, int mode) {
    // struct tm time;
...
static int fat32_unlink_name(struct inode* dir, struct dentry* dentry) {
    buf_head* bh = NULL;
    struct fat_dir_slot* ds;
...
int fat32_unlink(struct inode* dir, struct dentry* dentry) {
...
int fat32_read(struct file_desc* file, unsigned int count, char* buf) {
...
int fat32_write(struct file_desc* file, unsigned int count, const char* buf) {
    struct inode* inode = file->fd_dentry->d_inode;
    u64 start, pos, end, len;
    buf_head* bh = NULL;
    char* file_buf;
...
int fat32_fill_superblock(struct super_block* sb, int dev) {
    ...
    sbi->max_cluster = (sbi->tot_block - sbi->data_start_block) / sbi->cluster_block + 1;
    ...
    sbi->fsinfo =
        *((struct fat_fsinfo*)((char*)fs_info_buf->buffer + (sb->sb_blocksize * sbi->fsinfo_block) +
