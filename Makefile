...
#sudo mkfs.vfat -F 32 $(FREE_LOOP)p5;
	sudo mkfs.vfat -F 32 $(FREE_LOOP)5;
...
#cp ./hd/$(BOOT_IMG) ./b.img	
	@if [[ "$(USING_GRUB_CHAINLOADER)" != "true" ]]; then \
		dd if=os/boot/mbr/mbr.bin of=/dev/sda bs=1 count=446 conv=notrunc ; \
	fi
#	dd if=os/boot/mbr/orangefs_boot.bin of=b.img bs=1 count=512 seek=$(OSBOOT_OFFSET) conv=notrunc

#sudo losetup -P $(FREE_LOOP) b.img
	sudo $(BOOT_PART_FS_MAKER) $(BOOT_PART_FS_MAKE_FLAG) $(BOOT_PART)  #$(FREE_LOOP)2	# modified by mingxuan 2021-2-28

# FAT322规范规定第90~512个字节(共423个字节)是引导程序 # added by mingxuan 2020-10-5
	dd if=os/boot/mbr/$(BOOT) of=/dev/sda bs=1 count=$(BOOT_SIZE) seek=$(OSBOOT_START_OFFSET) conv=notrunc
#	dd if=os/boot/mbr/orangefs_boot.bin of=b.img bs=1 count=512 seek=$(OSBOOT_OFFSET) conv=notrunc
	#初始化根文件系统
